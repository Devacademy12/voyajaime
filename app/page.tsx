    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'DM Sans',sans-serif;background:#FAFAF9;overflow-x:hidden}
        ...
        .hero-content { position:absolute; top:50%; left:72px; transform:translateY(-54%); max-width:560px; }
        ...
        @media(max-width:900px){
          ...
          .footer-inner { flex-direction:column; gap:18px; text-align:center; padding:28px 28px; }
        }
        ...
        @media(max-width:600px){
          ...
          .nav-mobile { position:fixed; top:68px; left:0; right:0; background:white; border-bottom:1px solid #E5E7EB; padding:16px 20px; display:flex; flex-direction:column; gap:14px; z-index:199; box-shadow:0 8px 24px rgba(0,0,0,0.08); }
      ...
      {/* ── NAVBAR ── */}
      <header style={{ position:"fixed", top:0, left:0, right:0, zIndex:200, height:68, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 24px", background:scrolled?"rgba(255,255,255,0.97)":"transparent", backdropFilter:scrolled?"blur(20px)":"none", borderBottom:scrolled?"1px solid rgba(0,0,0,0.07)":"none", transition:"all 0.35s" }}>
        ...
        <nav className="nav-links">
          ...
          {user ? (
            <Link href="/touriste/favoris" style={{ position:"relative", textDecoration:"none", color:scrolled?"#374151":"rgba(255,255,255,0.88)", fontSize:14, fontWeight:500, display:"flex", alignItems:"center", gap:5 }}>
              ...
              {favCount > 0 && <span style={{ position:"absolute", top:-6, right:-10, background:"#DC2626", color:"white", borderRadius:"50%", width:16, height:16, fontSize:10, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center" }}>{favCount}</span>}
            </Link>
          ) : (
          ...
          {user
            ? <Link href="/touriste/reservations" style={{ padding:"10px 20px", background:scrolled?"#2B96A8":"rgba(255,255,255,0.18)", border:scrolled?"none":"1.5px solid rgba(255,255,255,0.45)", color:"white", borderRadius:30, textDecoration:"none", fontSize:14, fontWeight:700, backdropFilter:"blur(8px)", display:"inline-flex", alignItems:"center", gap:6 }}><User size={14} /> Mon espace</Link>
            : <Link href="/auth" style={{ padding:"10px 22px", background:scrolled?"#2B96A8":"rgba(255,255,255,0.18)", border:scrolled?"none":"1.5px solid rgba(255,255,255,0.45)", color:"white", borderRadius:30, textDecoration:"none", fontSize:14, fontWeight:700, backdropFilter:"blur(8px)", display:"inline-flex", alignItems:"center", gap:6 }}><LogIn size={14} /> Connexion</Link>
          }
        </nav>
      ...
      <div className={`nav-mobile ${navOpen ? "" : "closed"}`}>
        <Link href="/excursions" onClick={() => setNavOpen(false)} style={{ fontSize:15, fontWeight:500, color:"#374151", textDecoration:"none", display:"flex", alignItems:"center", gap:8 }}>
          <Map size={16} color="#9CA3AF" /> Excursions
        </Link>
        <a href="#chemins" onClick={() => setNavOpen(false)} style={{ fontSize:15, fontWeight:500, color:"#374151", textDecoration:"none", display:"flex", alignItems:"center", gap:8 }}>
          <Sparkles size={16} color="#9CA3AF" /> Comment ça marche
        </a>
        <Link href={user ? "/touriste/favoris" : "/auth"} onClick={() => setNavOpen(false)} style={{ fontSize:15, fontWeight:500, color:"#374151", textDecoration:"none", display:"flex", alignItems:"center", gap:8 }}>
          <Heart size={16} color="#9CA3AF" /> Favoris {favCount > 0 && `(${favCount})`}
        </Link>
        <Link href={user ? "/touriste/reservations" : "/auth"} onClick={() => setNavOpen(false)}
          style={{ padding:"12px 20px", background:"#2B96A8", color:"white", borderRadius:12, textDecoration:"none", fontSize:14, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>
      ...
      <section style={{ position:"relative", height:"100vh", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", inset:0, background:"linear-gradient(to right,rgba(0,0,0,0.72) 0%,rgba(0,0,0,0.3) 55%,rgba(0,0,0,0.05) 100%)" }} />
        ...
        <div key={current} className="hero-content">
          ...
          <h1 className="fu fu2" style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(34px,5.5vw,68px)", fontWeight:900, color:"white", lineHeight:1.08, letterSpacing:"-1.5px", marginBottom:16, textShadow:"0 2px 24px rgba(0,0,0,0.25)" }}>
            {slide.city}
          </h1>
          <p className="fu fu3" style={{ fontSize:"clamp(14px,2vw,17px)", color:"rgba(255,255,255,0.82)", lineHeight:1.65, marginBottom:32 }}>{slide.desc}</p>
          <div className="fu fu4 hero-btns">
            <a href="#chemins" style={{ padding:"13px 24px", background:"#2B96A8", color:"white", borderRadius:12, textDecoration:"none", fontSize:14, fontWeight:700, boxShadow:"0 8px 28px rgba(43,150,168,0.5)", display:"inline-flex", alignItems:"center", gap:8 }}>
              <Sparkles size={16} /> Planifier mon voyage
            </a>
            <Link href="/excursions" style={{ padding:"13px 22px", background:"rgba(255,255,255,0.13)", backdropFilter:"blur(12px)", border:"1.5px solid rgba(255,255,255,0.38)", color:"white", borderRadius:12, textDecoration:"none", fontSize:14, fontWeight:600, display:"inline-flex", alignItems:"center", gap:8 }}>
        ...
        <div className="hero-dots">
          {SLIDES.map((_, i) => (
            <button key={i} onClick={() => goTo(i)} style={{ background:"none", border:"none", cursor:"pointer", padding:4 }}>
              {i === current
                ? <div style={{ width:36, height:6, borderRadius:3, background:slide.color, overflow:"hidden" }}><div style={{ height:"100%", width:`${progress}%`, background:"rgba(255,255,255,0.6)", transition:"width 0.06s linear" }} /></div>
      ...
      <section id="chemins" style={{ position:"relative", overflow:"hidden" }}>
        ...
        <div className="chemins-wrap" style={{ position:"relative", zIndex:1 }}>
          <div style={{ textAlign:"center", marginBottom:52 }}>
            ...
            <p style={{ marginTop:14, fontSize:15, color:"rgba(255,255,255,0.6)" }}>Trouvez votre aventure idéale ou composez votre séjour sur mesure</p>
          </div>

          <div className="paths-grid">
            {[
              { bg:"rgba(43,150,168,0.18)", bc:"rgba(43,150,168,0.4)", badge:<><Sparkles size={11} color="#7EDCED"/><span style={{fontSize:10,fontWeight:800,color:"#7EDCED",letterSpacing:0.8}}>RAPIDE</span></>, icon:<Bot size={36} color="rgba(255,255,255,0.85)"/>, title:"Mode Assisté", sub:"Rapide & Intelligent", subColor:"#7EDCED", desc:"Répondez à 3 questions, on génère votre itinéraire jour par jour automatiquement", href:user?"/touriste/itineraire?mode=assiste":"/auth?redirect=itineraire", btnStyle:{background:"#2B96A8",border:"none",boxShadow:"0 6px 20px rgba(43,150,168,0.45)"}, btnLabel:"Je veux qu'on me propose" },
              { bg:"rgba(255,255,255,0.1)", bc:"rgba(255,255,255,0.25)", badge:<><Map size={11} color="rgba(255,255,255,0.65)"/><span style={{fontSize:10,fontWeight:800,color:"rgba(255,255,255,0.65)",letterSpacing:0.8}}>FLEXIBLE</span></>, icon:<ClipboardList size={36} color="rgba(255,255,255,0.85)"/>, title:"Mode Libre", sub:"Totalement personnalisé", subColor:"rgba(255,255,255,0.5)", desc:"Construisez votre voyage de A à Z — choisissez chaque excursion, chaque jour", href:user?"/touriste/itineraire?mode=libre":"/auth?redirect=itineraire", btnStyle:{background:"rgba(255,255,255,0.15)",border:"1.5px solid rgba(255,255,255,0.35)"}, btnLabel:"Je veux décider moi-même" },
              { bg:"rgba(255,255,255,0.06)", bc:"rgba(255,255,255,0.18)", badge:<><Search size={11} color="rgba(255,255,255,0.55)"/><span style={{fontSize:10,fontWeight:800,color:"rgba(255,255,255,0.55)",letterSpacing:0.8}}>EXPLORER</span></>, icon:<Mountain size={36} color="rgba(255,255,255,0.85)"/>, title:"Juste Explorer", sub:"Naviguez librement", subColor:"rgba(255,255,255,0.45)", desc:"Parcourez toutes les excursions disponibles — pas besoin de compte pour regarder", href:"/excursions", btnStyle:{background:"transparent",border:"1.5px solid rgba(255,255,255,0.25)"}, btnLabel:"Voir toutes les excursions" },
            ].map((c, i) => (
              <div key={i} className="path-card" style={{ background:c.bg, borderColor:c.bc }}>
                ...
                <p style={{ fontSize:13.5, color:"rgba(255,255,255,0.72)", lineHeight:1.6 }}>{c.desc}</p>
                <Link href={c.href} style={{ display:"inline-flex", alignItems:"center", gap:7, padding:"11px 20px", color:"white", borderRadius:12, textDecoration:"none", fontSize:13, fontWeight:700, marginTop:"auto", ...c.btnStyle }}>
        ...
        <div className="section-wrap" style={{ maxWidth:1160, margin:"0 auto" }}>
          <div className="exc-section-header">
            <div>
              ...
              <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(24px,3.5vw,36px)", fontWeight:900, color:"#111827", letterSpacing:"-0.5px" }}>Les plus populaires</h2>
            </div>
            <Link href="/excursions" style={{ fontSize:14, color:"#2B96A8", fontWeight:600, textDecoration:"none", display:"inline-flex", alignItems:"center", gap:5, whiteSpace:"nowrap" }}>
                    ...
                    <div style={{ padding:16 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                        <div style={{ minWidth:0 }}>
                          <h3 style={{ fontSize:15, fontWeight:700, color:"#111827", marginBottom:2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{exc.title}</h3>
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12, flexWrap:"wrap", gap:8 }}>
                        ...
                        <div style={{ padding:"7px 14px", background:user?"#2B96A8":"#E5E7EB", color:user?"white":"#6B7280", borderRadius:8, fontSize:12, fontWeight:700, display:"inline-flex", alignItems:"center", gap:5, flexShrink:0 }}>
          ...
          {!user && (
            <div style={{ marginTop:48, padding:"28px 32px", background:"linear-gradient(135deg,#EFF9FB,#F0FFFE)", border:"1.5px solid #B2E3EB", borderRadius:20 }}>
              <div className="guest-cta">
                <div>
                  <h3 style={{ fontSize:18, fontWeight:700, color:"#111827", marginBottom:6 }}>Envie de sauvegarder ou réserver ?</h3>
                ...
                <div className="guest-cta-btns">
                  <Link href="/auth" style={{ padding:"12px 24px", background:"#2B96A8", color:"white", borderRadius:12, textDecoration:"none", fontSize:14, fontWeight:700, boxShadow:"0 4px 14px rgba(43,150,168,0.35)", display:"inline-flex", alignItems:"center", gap:7, whiteSpace:"nowrap" }}>
                    <UserPlus size={15} /> Créer un compte
                  </Link>
                  <Link href="/auth" style={{ padding:"12px 20px", background:"white", border:"1.5px solid #E5E7EB", color:"#374151", borderRadius:12, textDecoration:"none", fontSize:14, fontWeight:600, display:"inline-flex", alignItems:"center", gap:7, whiteSpace:"nowrap" }}>
      ...
      <footer style={{ background:"#0D1117" }}>
        <div className="footer-inner">
          ...
          <div style={{ display:"flex", gap:16, alignItems:"center" }}>
            ...
            {user
              ? <Link href="/touriste/reservations" style={{ padding:"9px 18px", background:"#2B96A8", color:"white", borderRadius:30, textDecoration:"none", fontSize:13, fontWeight:700, display:"inline-flex", alignItems:"center", gap:6 }}><User size={13} /> Mon espace</Link>
              : <Link href="/auth" style={{ padding:"9px 18px", background:"#2B96A8", color:"white", borderRadius:30, textDecoration:"none", fontSize:13, fontWeight:700, display:"inline-flex", alignItems:"center", gap:6 }}><LogIn size={13} /> Connexion</Link>
            }
          </div>
        </div>
      </footer>
