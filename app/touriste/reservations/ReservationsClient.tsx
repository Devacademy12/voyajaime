...
import {
  ...
  CheckCircle, XCircle, CreditCard, Wallet, Building2,
...
function CheckoutModal({ reservation, onClose, onPaid }: {
    ...
    try {
      ...
      if (e1) {
        const { error: e2 } = await supabase.from("reservations")
          .update({ status: "confirmed" }).eq("id", reservation.id);
        if (e2) throw e2;
      }
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(17,24,39,.7)", backdropFilter: "blur(8px)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <button onClick={() => setStep(2)}
                style={{ width: "100%", padding: "15px", background: "#111827", color: "white", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Handicap, allergies, préférences particulières, demandes spéciales…"
                  rows={4}
          ...
          {step === 3 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <button onClick={handlePay} disabled={loading}
                ...
                {loading
                  ? <><Loader size={17} style={{ animation: "rspin 1s linear infinite" }} /> Traitement en cours…</>
                  : <><CheckCircle size={17} strokeWidth={2.5} /> Confirmer le paiement · {total} TND</>
                }
              </button>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20, paddingTop: 8 }}>
              ...
              <div style={{ textAlign: "center", padding: "20px 0 10px" }}>
                ...
                <p style={{ fontSize: 14, color: "#6B7280", lineHeight: 1.7 }}>
                  Votre réservation est confirmée. Retrouvez tous les détails ci-dessous.
                </p>
              </div>
              ...
              <div style={{ border: "2px solid #E5E7EB", borderRadius: 20, overflow: "hidden" }}>
                ...
                {/* Ticket perforation */}
                <div style={{ display: "flex", alignItems: "center", gap: 0, padding: "0 10px", borderTop: "2px dashed #E5E7EB" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={onClose}
                  style={{ flex: 1, padding: "14px", background: "#F3F4F6", color: "#374151", border: "none", borderRadius: 14, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                  Fermer
                </button>
                <Link href="/touriste/reservations" onClick={onClose}
                  style={{ flex: 2, padding: "14px", background: "#111827", color: "white", borderRadius: 14, textDecoration: "none", fontSize: 14, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
...
function ReservationCard({ r, onPay }: { r: Reservation; onPay: () => void }) {
  ...
  return (
    <div style={{ background: "white", borderRadius: 20, border: "1px solid #E5E7EB", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.05)", transition: "box-shadow .2s, transform .2s" }}
    ...
    >
      <div style={{ display: "flex" }}>
        {/* Photo */}
        <div style={{ width: 130, flexShrink: 0, position: "relative", overflow: "hidden", background: "linear-gradient(135deg,#2B96A8,#0e7490)" }}>
          {photo
            ? <img src={photo} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <CalendarDays size={28} color="rgba(255,255,255,.4)" />
              </div>
          }
          {/* Overlay gradient */}
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, transparent 60%, rgba(0,0,0,.15))" }} />
        </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 10 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", background: s.bg, border: `1px solid ${s.border}`, borderRadius: 99, fontSize: 11, fontWeight: 700, color: s.color }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: s.dot, flexShrink: 0 }} />
                  {s.label}
                </span>
                {paid && (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", background: "#D1FAE5", border: "1px solid #6EE7B7", borderRadius: 99, fontSize: 11, fontWeight: 700, color: "#065F46" }}>
                    <CreditCard size={10} strokeWidth={2.5} /> Payée
                  </span>
                )}
              </div>
              <h3 style={{ fontSize: 15, fontWeight: 800, color: "#111827", marginBottom: 6, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {exc?.title ?? "Excursion"}
              </h3>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 12px" }}>
                <span style={{ fontSize: 12, color: "#6B7280", display: "flex", alignItems: "center", gap: 3 }}>
                  <MapPin size={11} color="#9CA3AF" strokeWidth={1.5} /> {exc?.city}
                </span>
                <span style={{ fontSize: 12, color: "#6B7280", display: "flex", alignItems: "center", gap: 3 }}>
                  <CalendarDays size={11} color="#9CA3AF" strokeWidth={1.5} /> {fmtDate(r.date, true)}
                </span>
                <span style={{ fontSize: 12, color: "#6B7280", display: "flex", alignItems: "center", gap: 3 }}>
                  <Users size={11} color="#9CA3AF" strokeWidth={1.5} /> {r.people_count} pers.
                </span>
                <span style={{ fontSize: 12, color: "#6B7280", display: "flex", alignItems: "center", gap: 3 }}>
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <p style={{ fontSize: 18, fontWeight: 900, color: "#111827", lineHeight: 1 }}>{r.total_price}</p>
              <p style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 500, marginTop: 2 }}>TND</p>
            </div>
          </div>
      ...
      {!paid && (
        <div style={{ borderTop: "1px solid #F3F4F6", padding: "14px 20px" }}>
          <button onClick={onPay}
            style={{ width: "100%", padding: "12px 20px", background: "#111827", color: "white", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background .2s" }}
...
export default function ReservationsClient({
  reservations: init,
  total: t0, pending: p0, confirmed: c0,
}: {
    ...
    <div>
      ...
      {total > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 28 }}>
          {[
            { label: "Réservations", value: total,     color: "#2B96A8", bg: "#EFF9FB", border: "#BAE6FD" },
            { label: "En attente",   value: pending,   color: "#D97706", bg: "#FFFBEB", border: "#FDE68A" },
            { label: "Confirmées",   value: confirmed, color: "#059669", bg: "#ECFDF5", border: "#6EE7B7" },
          ].map(s => (
            <div key={s.label} style={{ background: s.bg, borderRadius: 16, padding: "16px 18px", border: `1px solid ${s.border}` }}>
              <p style={{ fontSize: 28, fontWeight: 900, color: s.color, lineHeight: 1, marginBottom: 4 }}>{s.value}</p>
              <p style={{ fontSize: 12, color: "#6B7280", fontWeight: 600 }}>{s.label}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {total === 0 ? (
        <div style={{ textAlign: "center", padding: "80px 20px", background: "white", borderRadius: 24, border: "1px solid #E5E7EB" }}>
          ...
          <Link href="/excursions"
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 26px", background: "#2B96A8", color: "white", borderRadius: 14, textDecoration: "none", fontSize: 14, fontWeight: 700, boxShadow: "0 4px 14px rgba(43,150,168,.35)" }}>
