...
const RESPONSIVE_CSS = `
  ...
  .resa-wrap { font-family:'DM Sans',system-ui,sans-serif; width:100%; box-sizing:border-box; }
  ...
  .resa-stats { display:flex; gap:12px; margin-bottom:32px; flex-wrap:wrap; }
  .resa-stat-card {
    display:flex; align-items:center; gap:12px;
    padding:14px 20px; border-radius:16px;
    min-width:130px; flex:1; border:1.5px solid; box-sizing:border-box;
  }
  .resa-stat-icon {
  ...
  .resa-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(340px,1fr)); gap:20px; }
  ...
  .resa-empty-icon {
    ...
    margin:0 auto 20px; border:2px solid rgba(43,150,168,.25);
  }

  .resa-refresh-btn {
    display:flex; align-items:center; gap:6px;
    padding:9px 16px; border-radius:10px;
    border:1.5px solid rgba(43,150,168,.25); background:#FFFFFF;
  ...
  .resa-cta-primary {
    ...
    background:linear-gradient(135deg,#2B96A8,#053366); color:white;
    border-radius:40px; text-decoration:none; font-size:14px; font-weight:700;
    box-shadow:0 4px 16px rgba(43,150,168,.35); transition:all .18s;
  }
  ...
  .resa-cta-secondary {
    ...
    background:#FFFFFF; color:#6B7A8D; border:1.5px solid rgba(5,51,102,.12);
    border-radius:40px; text-decoration:none; font-size:14px; font-weight:600; transition:all .15s;
  }
  .resa-cta-secondary:hover { background:rgba(5,51,102,.04); color:#053366; }

  @media (max-width:900px) {
    .resa-grid { grid-template-columns:repeat(auto-fill,minmax(300px,1fr)); gap:16px; }
  }
  @media (max-width:640px) {
    ...
    .resa-cta-primary,.resa-cta-secondary { justify-content:center; width:100%; box-sizing:border-box; }
  }
...
function isReservationDateTimePassed(r: Reservation): boolean {
  const timeStr = r.time
    ...
    : "23:59";
  return new Date(`${r.date}T${timeStr}:00`).getTime() < Date.now();
}
...
// ✅ FIX : vérifier qu'une réservation pending est encore active (deadline non dépassée)
function hasActivePendingPayment(r: Reservation): boolean {
  if (r.status !== "pending") return false;
  if (!r.payment_deadline) return true; // pas de deadline = toujours actif
  return new Date(r.payment_deadline).getTime() > Date.now();
}
...
export default function ReservationsClient({ reservations: init, autoOpenId }: Props) {
  ...
  // ✅ FIX PRINCIPAL : setInterval ne tourne QUE si au moins une réservation
  //    pending a encore une deadline valide (non expirée)
  //    → stoppe la boucle infinie de PATCH 404
  useEffect(() => {
    const hasRealPending = reservations.some(hasActivePendingPayment);
    if (!hasRealPending) return; // ← pas d'interval si rien d'actif

    const interval = setInterval(() => {
      const stillHasPending = reservations.some(hasActivePendingPayment);
      if (stillHasPending) refreshReservations();
    }, 30000);
    ...
    <>
      ...
      <div className="resa-wrap">
        ...
        <div className="resa-page-header">
          <div>
            <p style={{ fontSize:12, fontWeight:700, color:"#2B96A8", letterSpacing:"0.08em", textTransform:"uppercase", margin:"0 0 6px" }}>
              Mes voyages
            </p>
            <h1 style={{ fontSize:"clamp(20px,4vw,28px)", fontWeight:700, color:"#053366", margin:"0 0 6px" }}>
              Réservations
            </h1>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            {isRefreshing && (
              <span style={{ fontSize:12, color:"#2B96A8", display:"flex", alignItems:"center", gap:5 }}>
                <Loader2 size={12} style={{ animation:"spin 1s linear infinite" }} />
                Mise à jour…
              </span>
            )}
            <button className="resa-refresh-btn" onClick={refreshReservations} disabled={isRefreshing}>
        ...
        {total > 0 && (
          <div className="resa-stats">
            {stats.map(({ icon, num, lbl, bg, border, numColor }) => (
              <div key={lbl} className="resa-stat-card" style={{ background:bg, borderColor:border }}>
                <div className="resa-stat-icon">{icon}</div>
                <div>
                  <p style={{ fontSize:22, fontWeight:800, color:numColor, margin:0, lineHeight:1 }}>{num}</p>
                  <p style={{ fontSize:11, color:"#9CA3AF", margin:"3px 0 0", fontWeight:600 }}>{lbl}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {total === 0 ? (
          <div className="resa-empty">
            ...
            <h3 style={{ fontSize:"clamp(17px,4vw,20px)", fontWeight:700, color:"#053366", marginBottom:10 }}>
              Aucune réservation active
            </h3>
            <p style={{ fontSize:14, color:"#9CA3AF", lineHeight:1.7, maxWidth:340, margin:"0 auto 32px" }}>
              Découvrez les excursions et planifiez votre prochain voyage en Tunisie
            </p>
            <div className="resa-cta-row" style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
          ...
          <div className="resa-grid">
            {reservations.map((r, i) => (
              <div key={r.id} className="resa-grid-item" style={{ animationDelay:`${i * 0.08}s` }}>
                <ReservationCard
                  r={r}
                  onPay={() => setCheckout(r)}
