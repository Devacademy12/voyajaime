...
.ma2-toast {
  ...
  font-size: 14px; font-weight: 600; font-family: 'DM Sans', system-ui, sans-serif;
  z-index: 9999; pointer-events: none;
  box-shadow: 0 8px 32px rgba(0,0,0,.18);
  animation: ma2toastIn .3s cubic-bezier(.22,.68,0,1.2) both;
  white-space: nowrap;
}
@keyframes ma2toastIn { from{opacity:0;transform:translateX(-50%) translateY(16px)} to{opacity:1;transform:translateX(-50%) translateY(0)} }
.ma2-toast.ok      { background: #052e16; color: #bbf7d0; }
.ma2-toast.error   { background: #450a0a; color: #fecaca; }
.ma2-toast.login   { background: #1e293b; color: #e2e8f0; }
.ma2-toast.saving  { background: #0c1a2e; color: #bae6fd; }
...
@media (max-width: 640px) {
  ...
  .ma2-toast { font-size: 12px; padding: 11px 16px; bottom: 16px; max-width: calc(100vw - 32px); white-space: normal; }
}
`;
...
function Toast({ status, onDone }: { status: "idle"|"ok"|"error"|"login"|"saving"; onDone: () => void }) {
  ...
  const config = {
    ...
    ok:     { icon: "✅", msg: "Itinéraire sauvegardé avec succès !" },
    error:  { icon: "❌", msg: "Erreur lors de la sauvegarde. Réessayez." },
    login:  { icon: "🔐", msg: "Connectez-vous pour sauvegarder votre itinéraire." },
  }[status];

  return createPortal(
    <div className={`ma2-toast ${status}`}>
