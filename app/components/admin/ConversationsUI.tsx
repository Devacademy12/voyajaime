...
export interface Excursion { id: string; title: string; }
export interface Message {
  id: string; conversation_id: string; contenu: string;
  lu: boolean; expediteur_id: string; created_at: string;
}
export interface Profile { user_id: string; full_name: string | null; avatar_url: string | null; }
export interface Violation { type: string; label: string; match: string; }

/* ── Icône par type de violation ── */
export function ViolationIcon({ type }: { type: string }) {
...
export function Avatar({ url, name, size = 40, radius = 12 }: { url?: string | null; name?: string | null; size?: number; radius?: number }) {
...
export function HeaderSection({ conversations, totalUnread, totalAlerts }: { conversations: Conv[], totalUnread: number, totalAlerts: number }) {
  return (
...
export function AlertBanner({ totalAlerts, onViewAlerts }: { totalAlerts: number, onViewAlerts: () => void }) {
  return (
...
export function StatsGrid({ conversations, totalUnread, messages, totalAlerts }: { conversations: Conv[], totalUnread: number, messages: Message[], totalAlerts: number }) {
  return (
...
export function ConversationList({
  ...
  filterAlert: boolean,
  setFilterAlert: (value: boolean) => void,
  filtered: Conv[],
  msgByConv: Record<string, Message[]>,
  alertConvIds: Set<string>,
  selected: string | null,
  setSelected: (id: string | null) => void,
  profileMap: Record<string, Profile>,
  excMap: Record<string, string>,
  formatDate: (d: string) => string,
  totalAlerts: number,
  conversations: Conv[]
}) {
  return (
    <div className="cv-list-pane" style={{ width:500, borderRight:"1px solid #EEF2FF", display:"flex", flexDirection:"column", flexShrink:0 }}>
      ...
      <div style={{ flex:1, overflowY:"hidden", padding:"8px", minHeight:0 }}>
        {filtered.length === 0 ? (
...
export function MessageItem({
  ...
  formatDate: (d: string) => string,
  highlightViolations: (contenu: string, msgViolations: Violation[]) => JSX.Element
}) {
  const isTouriste    = message.expediteur_id === selectedConv.touriste_id;
  const senderProfile = isTouriste ? profileMap[selectedConv.touriste_id] : profileMap[selectedConv.prestataire_id];
  const senderName    = isTouriste
    ? (selectedConv.touriste_name    || "Touriste")
    : (selectedConv.prestataire_name || "Prestataire");
  const msgViolations = violations[message.id] || [];
  const hasViolation  = msgViolations.length > 0;

  return (
    <div key={message.id} className="cv-msg-fadein" style={{ display:"flex", flexDirection:"column", alignItems:isTouriste ? "flex-start" : "flex-end", animationDelay:`${index * .025}s` }}>
      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:5, flexDirection:isTouriste ? "row" : "row-reverse" }}>
        ...
        <span style={{ fontSize:11, color:"#9CA3AF" }}>{formatDate(message.created_at)}</span>
        {!message.lu && <span style={{ width:6, height:6, borderRadius:"50%", background:"#EF4444", display:"inline-block" }}/>}
      </div>

      <div className={`msg-bubble ${isTouriste ? "msg-t" : "msg-p"}`}
        style={ hasViolation ? { outline:"2px solid #FCA5A5", outlineOffset:2 } : undefined }>
        {highlightViolations(message.contenu, msgViolations)}
      </div>
...
export function MessagesArea({
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"20px", display:"flex", flexDirection:"column", gap:14, background:"#F8FAFF", minHeight:0 }}>
