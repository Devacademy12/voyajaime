...
import {
  ...
  Globe, Hash, Building2, FileText, ExternalLink, Image as ImageIcon,
  ShieldCheck, Mail,
} from "lucide-react";
    ...
    <div style={{ display: "flex", gap: 6 }}>
      {([
        ...
        >
          ...
          {label}
          <span style={{
            fontSize: 11, borderRadius: 12, padding: "1px 7px", fontWeight: 800,
...
export function EmptyPrestataires({ filter }: { filter: string }) {
  return (
    <div style={{ textAlign: "center", padding: "56px 20px", background: "white", borderRadius: 16, border: "1px solid #F3F4F6" }}>
      ...
      {filter === "pending" && (
        <p style={{ color: "#9CA3AF", fontSize: 13, marginTop: 5 }}>Toutes les demandes ont été traitées !</p>
      )}
    </div>
  );
}
...
export function PrestastaireCard({
  p, isLoading, onViewEdit, onValidate, onRevoke, onDelete,
}: {
  ...
  return (
    <div className="pcard" style={{ cursor: "pointer" }} onClick={onViewEdit}>
      {/* Avatar */}
      <div style={{
        width: 44, height: 44, borderRadius: "50%", overflow: "hidden", flexShrink: 0,
        background: p.is_validated
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3, flexWrap: "wrap" }}>
          ...
          <span style={{
            padding: "2px 8px", borderRadius: 18, fontSize: 11, fontWeight: 700,
          ...
          {p.excursion_count > 0 && (
            <span style={{
              padding: "2px 8px", borderRadius: 18, fontSize: 11, fontWeight: 600,
              background: "rgba(43,150,168,.08)", color: "#2B96A8",
              display: "inline-flex", alignItems: "center", gap: 4,
            }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          {p.email && (
            <span style={{ fontSize: 12, color: "#6B7280", display: "flex", alignItems: "center", gap: 4 }}>
              <Mail size={11} color="#9CA3AF" /> {p.email}
            </span>
          )}
...
export function PrestastaireDetails({
  p, loading, onEdit, onValidate, onRevoke, onDelete,
}: {
  p: PrestastaireUIProps;
  loading: boolean;
  onEdit: () => void;
  ...
  return (
    <div style={{ padding: "0 26px 26px" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {p.full_name && p.agency_name && (
            <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 7, display: "flex", alignItems: "center", gap: 5 }}>
              <Users size={12} color="#9CA3AF" /> {p.full_name}
            </p>
          )}
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <span style={{
              padding: "3px 9px", borderRadius: 18, fontSize: 11, fontWeight: 700,
              background: p.is_validated ? "#F0FDF4" : "#FFFBEB",
              color: p.is_validated ? "#15803D" : "#D97706",
            ...
            {p.excursion_count > 0 && (
              <span style={{ padding: "3px 9px", borderRadius: 18, fontSize: 11, fontWeight: 600, background: "rgba(43,150,168,.08)", color: "#2B96A8", display: "inline-flex", alignItems: "center", gap: 4 }}>
                <Map size={10} /> {p.excursion_active}/{p.excursion_count} excursions
              </span>
            )}
          </div>
        </div>
      ...
      <div style={{ marginBottom: 16 }}>
        {[
          { Icon: Mail,      label: "Email (compte)",  val: p.email   || "—" },
      ...
      <div style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 12, padding: "14px 16px", marginBottom: 16 }}>
        ...
        {p.declaration_url ? (
          <div style={{ marginTop: 10 }}>
            ...
            <a href={p.declaration_url} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "8px 14px", background: "white", border: "1px solid #BBF7D0", borderRadius: 9, fontSize: 12, fontWeight: 700, color: "#059669", textDecoration: "none" }}>
      ...
      <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
        <button className="pbtn pbtn-teal" style={{ flex: 1, padding: "11px", borderRadius: 9, fontFamily: "inherit" }} onClick={onEdit}>
          <Pencil size={13} /> Modifier
        </button>
        {!p.is_validated ? (
          <button className="pbtn pbtn-green" style={{ flex: 1, padding: "11px", borderRadius: 9, fontFamily: "inherit" }} onClick={onValidate} disabled={loading}>
            <CheckCircle size={13} /> {loading ? "..." : "Valider"}
          </button>
        ) : (
          <button className="pbtn pbtn-gray" style={{ flex: 1, padding: "11px", borderRadius: 9, fontFamily: "inherit" }} onClick={onRevoke} disabled={loading}>
            <AlertTriangle size={13} /> {loading ? "..." : "Révoquer"}
          </button>
        )}
        <button className="pbtn pbtn-red" style={{ padding: "11px 16px", borderRadius: 9, fontFamily: "inherit" }} onClick={onDelete} disabled={loading}>
          <Trash2 size={13} />
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ padding: "0 26px 26px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
        ...
        {/* ── Email (auth.users) ─────────────────────────────────────── */}
        <div style={{ background: "#EFF9FB", border: "1px solid #A5F3FC", borderRadius: 10, padding: "12px 14px" }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: "#0E7490", textTransform: "uppercase", letterSpacing: ".8px", marginBottom: 10, display: "flex", alignItems: "center", gap: 5 }}>
            <Mail size={11} color="#0E7490" /> Compte d'authentification Supabase
          </p>
          <label style={{ ...lbl, color: "#374151" }}>Adresse email</label>
          ...
          <p style={{ fontSize: 10, color: "#D97706", fontWeight: 600, marginTop: 6, display: "flex", alignItems: "center", gap: 4 }}>
            ⚠ Modifie directement le compte dans <span style={{ fontFamily: "monospace" }}>auth.users</span>
          </p>
        </div>
      ...
      >
        ...
        {mode === "view" ? (
          <PrestastaireDetails
            ...
            onValidate={onValidate} onRevoke={onRevoke} onDelete={onDelete}
          />
        ) : (
