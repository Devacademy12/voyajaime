...
interface ExcursionForCheckout {
  ...
  // Infos détectées depuis le plan
  plan_date?: string;       // date suggérée depuis le plan itinéraire
  plan_time?: "matin" | "aprem" | "soir"; // créneau du plan
  plan_day?: number;        // jour du plan
  note?: string;
}
...
/** Trouve le meilleur slot correspondant au créneau du plan (matin/aprem/soir) */
...
const CSS = `
  ...
  .cal-day.available:hover {
    background: rgba(43,150,168,.3); border-color: #2B96A8;
    color: white; transform: translateY(-1px);
  }
  .cal-day.selected {
    background: #2B96A8 !important; color: white !important;
    border-color: #2B96A8 !important;
    box-shadow: 0 4px 14px rgba(43,150,168,.45);
  ...
  .cmi-cta.on {
    background:linear-gradient(135deg,#2B96A8,#1a7a8f);
    color:white;
    box-shadow:0 4px 18px rgba(43,150,168,.3);
  }
  ...
  /* ── Progress bar ── */
  .prog-step {
    height:3px; border-radius:3px; flex:1; transition:background .3s;
  }
  ...
  .date-chip {
    ...
    font-size:11px; font-weight:600; color:#0E5068;
    cursor:pointer; transition:all .15s;
  }
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:12,marginTop:10,flexWrap:"wrap"}}>
        ...
        {suggestedDate&&<div style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"rgba(52,211,153,.8)"}}>
      ...
      {slots.map((slot,i)=>{
        ...
        return (
          <div key={i} className={`slot-pill ${isSel?"sel":""} ${!avail?"full":""}`}
            ...
            {avail&&<div style={{marginLeft:8,flexShrink:0}}>
...
export default function CheckoutModalItineraire({ excursions, itineraireId: propItinId, itineraireTitle, onClose }: Props) {
  const supabase = createClient();

  // Initialise perExc avec les dateMap et pré-sélection automatique des dates du plan
  const [perExc, setPerExc] = useState(() =>
    excursions.map(e => {
      const dateMap = buildDateMap(e);
      // Cherche si la date du plan est disponible dans la dateMap
      let preDate = "";
      let preSlot: TimeSlot | null = null;

      if (e.plan_date) {
        const planDate = e.plan_date;
        const slotsOnPlanDate = dateMap.get(planDate);
      ...
      // Si pas de date plan ou indispo, prendre la première dispo
      if (!preDate) {
        preDate = getFirstAvailableDate(dateMap);
        if (preDate) {
          const slots = dateMap.get(preDate) || [];
          preSlot = findBestSlot(slots, e.plan_time);
        }
      }
    ...
    try {
      ...
      for(const p of perExc) {
        ...
        const {error:insErr} = await supabase.from("reservations").insert([{
          touriste_id:user.id, excursion_id:p.exc.id, itineraire_id:itineraireId,
          date:p.selectedDate, time:p.selectedSlot.time, people_count:p.people,
          total_price:tot+fee, platform_fee:fee, status:"pending",
          special_needs:p.specialNeeds.trim()||null, booking_code:code,
          payment_status:"unpaid", payment_method:null, special_notes:null,
        }]);
              ...
              {confirmed.map((p,i)=>(
                <div key={i} style={{background:"linear-gradient(135deg,#EFF9FB,#E0F5FA)",border:"1px solid rgba(43,150,168,.2)",borderRadius:14,padding:"13px 16px",textAlign:"left"}}>
                  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
                    <div>
                      ...
                      <p style={{margin:"0 0 5px",fontSize:12,color:"#6B7280",display:"flex",alignItems:"center",gap:6}}>
                        ...
                        {p.exc.plan_time&&<span style={{display:"inline-flex",alignItems:"center",gap:3}}>
                  ...
                  <div style={{marginTop:8,paddingTop:8,borderTop:"1px dashed rgba(43,150,168,.2)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <span style={{fontSize:12,fontWeight:800,color:"#2B96A8"}}>{(p.selectedSlot!.price*p.people)+Math.round(p.selectedSlot!.price*p.people*.1)} EUR</span>
                  </div>
                </div>
              ))}
            </div>

            <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:"12px 16px",marginBottom:22}}>
            ...
            <div className="cmi-left-inner">
              ...
              <div style={{position:"relative",zIndex:1}}>
                <div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(43,150,168,.2)",border:"1px solid rgba(43,150,168,.35)",borderRadius:20,padding:"4px 10px",marginBottom:10}}>
                  ...
                  <span style={{fontSize:10,fontWeight:700,color:"#7CC4D1",textTransform:"uppercase",letterSpacing:"1px"}}>Itinéraire · {excursions.length} excursion{excursions.length>1?"s":""}</span>
                </div>
          ...
          <div className="cmi-right">
            <div className="cmi-right-scroll">
              ...
              {!cur.selectedDate ? (
                ...
                <div style={{marginBottom:18}}>
                  <p style={{fontSize:11,fontWeight:700,color:"#374151",marginBottom:10,display:"flex",alignItems:"center",gap:5,textTransform:"uppercase",letterSpacing:".5px"}}>
                    ...
                    {cur.selectedDate&&<span style={{fontWeight:400,textTransform:"none",letterSpacing:0,color:"#9CA3AF"}}>
              ...
              {cur.selectedSlot&&(
                <div style={{marginBottom:18}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12,padding:"8px 16px",border:"1.5px solid #E5E7EB",borderRadius:12,background:"white"}}>
                    <button className="cmi-counter-btn" disabled={cur.people<=1}
                      onClick={()=>patch(activeIdx,{people:cur.people-1})}>
                      <Minus size={14}/>
                    </button>
                    <span style={{flex:1,textAlign:"center",fontSize:24,fontWeight:900,color:"#111827",fontFamily:"'Cormorant Garamond',serif"}}>
                      {cur.people}
                    </span>
                    <button className="cmi-counter-btn" disabled={cur.people>=maxPeople}
                      onClick={()=>patch(activeIdx,{people:cur.people+1})}>
                      <Plus size={14}/>
                    </button>
                  </div>
                  <p style={{fontSize:11,color:"#9CA3AF",marginTop:5}}>
                    ...
                    {cur.selectedSlot.price&&<span style={{marginLeft:8,color:"#2B96A8",fontWeight:600}}>
              ...
              <div style={{marginBottom:8}}>
                ...
                <textarea className="cmi-textarea"
