...
/** Retourne true si l'excursion a au moins 1 créneau disponible à partir d'aujourd'hui */
function hasAvailableDates(exc: ExcursionForCheckout): boolean {
...
function findBestSlot(slots: TimeSlot[], planTime?: string): TimeSlot | null {
  ...
  const [min,max] = ranges[planTime] || [0,24];
...
function formatDateFR(dateStr: string, opts?: Intl.DateTimeFormatOptions) {
  if (!dateStr) return "";
  try {
    return new Date(dateStr).toLocaleDateString("fr-FR", opts || { weekday:"long", day:"numeric", month:"long" });
...
function hasTimeOverlap(
  date1:string, time1:string, dur1:number,
  date2:string, time2:string, dur2:number,
): boolean {
  if (date1 !== date2) return false;
  const s1=timeToMinutes(time1), e1=s1+dur1*60;
  const s2=timeToMinutes(time2), e2=s2+dur2*60;
  return s1<e2 && s2<e1;
}
...
const CSS = `
  ...
  .cmi-overlay{position:fixed;inset:0;z-index:1000;display:flex;align-items:center;justify-content:center;padding:12px;background:rgba(5,15,30,0.72);backdrop-filter:blur(10px);animation:cmiFade .22s ease;}
  .cmi-shell{font-family:'DM Sans',sans-serif;background:#F8F9FB;border-radius:28px;width:100%;max-width:960px;box-shadow:0 40px 100px rgba(0,0,0,.38),0 0 0 1px rgba(255,255,255,.06);overflow:hidden;display:flex;max-height:94vh;animation:cmiUp .32s cubic-bezier(.34,1.4,.64,1);}

  .cmi-left{width:360px;flex-shrink:0;background:linear-gradient(168deg,#071E3D 0%,#0B3D63 45%,#0A6080 100%);display:flex;flex-direction:column;position:relative;overflow:hidden;}
  .cmi-left-inner{flex:1;overflow-y:auto;padding:28px 26px;display:flex;flex-direction:column;gap:20px;}
  ...
  .cmi-left::before{content:'';position:absolute;top:-80px;right:-80px;width:240px;height:240px;border-radius:50%;background:radial-gradient(circle,rgba(43,150,168,.25) 0%,transparent 70%);pointer-events:none;}
  .cmi-left::after{content:'';position:absolute;bottom:-60px;left:-50px;width:200px;height:200px;border-radius:50%;background:radial-gradient(circle,rgba(11,126,163,.18) 0%,transparent 70%);pointer-events:none;}

  .cmi-right{flex:1;display:flex;flex-direction:column;overflow:hidden;background:#FAFBFC;}
  .cmi-right-scroll{flex:1;overflow-y:auto;padding:26px 26px 0;scrollbar-width:thin;scrollbar-color:#E5E7EB transparent;}
  ...
  .exc-tab{border-radius:12px;padding:10px 13px;cursor:pointer;display:flex;align-items:center;gap:9px;border:1px solid transparent;transition:all .16s;font-family:'DM Sans',sans-serif;font-size:12px;font-weight:600;color:rgba(255,255,255,.65);background:rgba(255,255,255,.05);text-align:left;width:100%;}
  ...
  .exc-tab.unavailable{opacity:.5;cursor:not-allowed;}
  .exc-tab-num{width:22px;height:22px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;background:rgba(255,255,255,.12);color:rgba(255,255,255,.7);transition:all .16s;}
  .exc-tab.active .exc-tab-num{background:#2B96A8;color:white;}

  .cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:3px;}
  .cal-day{aspect-ratio:1;border-radius:9px;border:none;font-size:12px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .14s;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:1px;position:relative;}
  ...
  .cal-day.unavailable::before{content:'';position:absolute;left:15%;right:15%;top:50%;height:1.5px;background:rgba(255,255,255,.18);transform:rotate(-12deg);}
  ...
  .slot-pill{display:flex;align-items:center;justify-content:space-between;padding:12px 15px;border-radius:14px;border:1.5px solid #E5E7EB;background:white;cursor:pointer;transition:all .15s;margin-bottom:8px;position:relative;}
  ...
  .cmi-counter-btn{width:36px;height:36px;border:none;border-radius:10px;background:#F3F4F6;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .14s;}
  .cmi-counter-btn:hover:not(:disabled){background:#E5E7EB;}
  .cmi-counter-btn:disabled{opacity:.3;cursor:not-allowed;}

  .cmi-cta{width:100%;padding:15px;border:none;border-radius:14px;font-size:15px;font-weight:800;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px;letter-spacing:.3px;}
  ...
  .sugg-badge{display:inline-flex;align-items:center;gap:5px;background:rgba(52,211,153,.15);border:1px solid rgba(52,211,153,.3);border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;color:#059669;}
  .date-chip{display:inline-flex;align-items:center;gap:5px;background:rgba(43,150,168,.12);border:1px solid rgba(43,150,168,.25);border-radius:10px;padding:5px 10px;font-size:11px;font-weight:600;color:#0E5068;cursor:pointer;transition:all .15s;}
  .date-chip:hover{background:rgba(43,150,168,.2);border-color:#2B96A8;}

  .cmi-textarea{width:100%;padding:10px 13px;min-height:60px;border:1.5px solid #E5E7EB;border-radius:12px;font-size:13px;font-family:'DM Sans',sans-serif;color:#374151;resize:none;outline:none;background:#F9FAFB;box-sizing:border-box;transition:border .15s;}
  .cmi-textarea:focus{border-color:#2B96A8;background:white;}

  /* Bannière excursion supprimée */
  .removed-banner{background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-radius:10px;padding:10px 14px;font-size:12px;color:#DC2626;display:flex;align-items:center;gap:7px;margin-bottom:6px;}
...
function MiniCalendar({ dateMap, selectedDate, onSelect, suggestedDate }: {
  dateMap: Map<string,TimeSlot[]>; selectedDate: string;
  onSelect:(d:string)=>void; suggestedDate?:string;
}) {
  const today = new Date(); today.setHours(0,0,0,0);
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        ...
        <span style={{fontSize:13,fontWeight:700,color:"white"}}>{MONTHS_FR[viewMonth]} {viewYear}</span>
        <button onClick={()=>{const d=new Date(viewYear,viewMonth+1);setViewYear(d.getFullYear());setViewMonth(d.getMonth());}}
          style={{background:"rgba(255,255,255,.1)",border:"none",borderRadius:8,width:28,height:28,cursor:"pointer",color:"white",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{display:"flex",gap:12,marginTop:10,flexWrap:"wrap"}}>
        {[{color:"#34D399",label:"Disponible"},{color:"rgba(255,255,255,.2)",label:"Indisponible",cross:true}].map(l=>(
          <div key={l.label} style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"rgba(255,255,255,.5)"}}>
            <div style={{width:8,height:8,borderRadius:2,background:l.color,position:"relative"}}>
              {l.cross&&(<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{width:"110%",height:1.5,background:"rgba(255,255,255,.4)",transform:"rotate(-12deg)"}}/></div>)}
            </div>
            {l.label}
          </div>
        ))}
        {suggestedDate&&(<div style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"rgba(52,211,153,.8)"}}><div style={{width:8,height:8,borderRadius:2,background:"transparent",border:"2px solid rgba(52,211,153,.6)"}}/>Date suggérée</div>)}
      </div>
    </div>
  );
}
...
function SlotList({slots,selected,onSelect}:{slots:TimeSlot[];selected:TimeSlot|null;onSelect:(s:TimeSlot)=>void;}) {
  return (
    <div>
      {slots.map((slot,i)=>{
        const avail=slot.slots>0; const isSel=selected===slot; const low=avail&&slot.slots<=3;
        return (
          <div key={i} className={`slot-pill ${isSel?"sel":""} ${!avail?"full":""}`} onClick={()=>avail&&onSelect(slot)}>
            ...
            <div>
              ...
              {slot.groupPrice&&(<div style={{fontSize:11,color:"#10B981",fontWeight:600,marginTop:2,display:"flex",alignItems:"center",gap:3}}><Users size={9}/> Tarif groupe</div>)}
            </div>
            <div style={{textAlign:"right"}}>
              {avail?(<><div style={{fontSize:16,fontWeight:900,color:"#2B96A8"}}>{slot.price} EUR <span style={{fontSize:11,color:"#9CA3AF",fontWeight:400}}>/pers.</span></div><div style={{fontSize:11,color:low?"#F59E0B":"#9CA3AF",fontWeight:low?700:400}}>{slot.slots} place{slot.slots>1?"s":""}</div></>)
              :<span style={{fontSize:13,fontWeight:700,color:"#EF4444"}}>Complet</span>}
            </div>
            {avail&&(<div style={{marginLeft:8,flexShrink:0}}><div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${isSel?"#2B96A8":"#D1D5DB"}`,background:isSel?"#2B96A8":"white",display:"flex",alignItems:"center",justifyContent:"center"}}>{isSel&&<div style={{width:8,height:8,borderRadius:"50%",background:"white"}}/>}</div></div>)}
          </div>
        );
      })}
    </div>
...
export default function CheckoutModalItineraire({
  ...
  /* ── Filtrer les excursions sans dates disponibles ── */
  const [excursions, setExcursions] = useState<ExcursionForCheckout[]>([]);
  const [removedExc, setRemovedExc] = useState<ExcursionForCheckout[]>([]);

  useEffect(() => {
    const available: ExcursionForCheckout[] = [];
    const removed:   ExcursionForCheckout[] = [];
    rawExcursions.forEach(exc => {
      if (hasAvailableDates(exc)) available.push(exc);
      else                        removed.push(exc);
    });
    setExcursions(available);
    setRemovedExc(removed);
  }, [rawExcursions]);

  /* ── Per-excursion state ── */
  const [perExc, setPerExc] = useState<Array<{
    exc: ExcursionForCheckout;
  ...
  const [resItinId,            setResItinId]            = useState<string|null>(null);

  const patch = useCallback((idx:number, p:any) =>
  ...
  const handleGoToPay = (reservationIds: string[]) => {
    ...
    if (onPayNow) { onClose?.(); onPayNow(reservationIds[0]); }
    else          { onClose?.(); router.push(`/touriste/reservations?pay=${reservationIds[0]}`); }
  };
  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      /* Créer le reservation_itineraires parent */
      const {data:resItin, error:resItinErr} = await supabase
        ...
        .select("id").single();
      if (resItinErr||!resItin) { setErrorMsg(`Erreur groupe réservation : ${resItinErr?.message??"inconnue"}`); setStatus("error"); return; }
      const reservationItineraireId = resItin.id;
      setResItinId(reservationItineraireId);
      ...
      for (const p of perExc) {
        ...
        const {data:conflicting,error:conflictErr} = await supabase
          .from("reservations").select("id,excursion_id,date,time,excursions(duration_hours)")
        ...
        for (const c of conflicting||[]) {
          ...
          if (hasTimeOverlap(p.selectedDate,p.selectedSlot.time,duration,c.date,c.time,otherDuration)) {
            setErrorMsg("⚠️ Chevauchement horaire avec une autre réservation."); setStatus("error"); return;
          }
        }
        ...
        const {data:inserted, error:insErr} = await supabase
          ...
          .insert([{
            ...
            itineraire_id:            itineraireId,
            reservation_itineraire_id: reservationItineraireId,
            date:                     p.selectedDate,
          ...
          .select("id").single();
        if (insErr||!inserted) { setErrorMsg(`Erreur insertion : ${insErr?.message??"inconnue"}`); setStatus("error"); return; }
  ...
  /* ── Écran vide si aucune excursion disponible ── */
  if (excursions.length === 0 && removedExc.length > 0 && status !== "success") {
    return (
      <>
        <style>{CSS}</style>
        <div className="cmi-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose?.();}}>
          <div className="cmi-shell" style={{maxWidth:460,display:"block",padding:"44px 36px",textAlign:"center",background:"white"}}>
            <div style={{width:72,height:72,borderRadius:"50%",background:"#FEF2F2",border:"2px solid #FCA5A5",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}}>
              <Trash2 size={32} color="#EF4444"/>
            </div>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:700,color:"#111827",marginBottom:10}}>
              Aucune date disponible
            </h2>
            <p style={{fontSize:13,color:"#6B7280",marginBottom:20}}>
              Toutes les excursions de cet itinéraire n'ont plus de créneaux disponibles.
            </p>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
              {removedExc.map(e=>(
                <div key={e.id} className="removed-banner">
                  <Trash2 size={12}/> {sanitizeText(e.title)} — {e.city}
                </div>
              ))}
            </div>
      ...
      <>
        ...
        <div className="cmi-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose?.();}}>
          <div className="cmi-shell" style={{maxWidth:520,display:"block",padding:"44px 40px",textAlign:"center",background:"white"}}>
            <div style={{width:80,height:80,borderRadius:"50%",background:"linear-gradient(135deg,#D1FAE5,#A7F3D0)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 22px",boxShadow:"0 8px 28px rgba(5,150,105,.25)",animation:"pulseGreen 2s ease-in-out infinite"}}>
            ...
            <p style={{fontSize:13,color:"#6B7280",marginBottom:18}}>
              ...
              {removedExc.length>0&&<span style={{color:"#F59E0B"}}> · {removedExc.length} supprimée{removedExc.length>1?"s":""} (indisponibles)</span>}
            </p>

            {/* Excursions supprimées */}
            {removedExc.length>0&&(
              <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:"10px 14px",marginBottom:16,textAlign:"left"}}>
                <p style={{fontSize:11,fontWeight:700,color:"#92400E",margin:"0 0 6px",display:"flex",alignItems:"center",gap:5}}>
                  <AlertCircle size={12}/> Excursions retirées (aucun créneau) :
                </p>
                {removedExc.map(e=>(
                  <p key={e.id} style={{fontSize:11,color:"#B45309",margin:"2px 0",display:"flex",alignItems:"center",gap:5}}>
                    <Trash2 size={10}/> {sanitizeText(e.title)}
                  </p>
                ))}
              </div>
            )}
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
                    <div>
                      ...
                      <p style={{margin:"0 0 5px",fontSize:12,color:"#6B7280",display:"flex",alignItems:"center",gap:6}}>
                        ...
                        {p.exc.plan_time&&(<span style={{display:"inline-flex",alignItems:"center",gap:3}}>{TIME_ICON[p.exc.plan_time]}{TIME_LABEL[p.exc.plan_time]}</span>)}
                      </p>
            ...
            {confirmed.length>1&&(
              <div style={{background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:12,padding:"12px 16px",marginBottom:16,textAlign:"left"}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#6B7280",marginBottom:4}}>
                  <span>Sous-total excursions</span><span>{subtotal} EUR</span>
                </div>
                ...
                <div style={{display:"flex",justifyContent:"space-between",fontSize:15,fontWeight:800,color:"#111827"}}>
                  <span>Total à payer</span><span style={{color:"#2B96A8"}}>{grandTotal} EUR</span>
                </div>
              </div>
            )}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <button className="cmi-cta" style={{background:"#F3F4F6",color:"#374151",fontSize:13}} onClick={()=>onClose?.()}>
          ...
          <div className="cmi-left">
            <div className="cmi-left-inner">
              ...
              <div style={{position:"relative",zIndex:1}}>
                ...
                {removedExc.length>0&&(
                  <div style={{fontSize:11,color:"rgba(251,191,36,.8)",display:"flex",alignItems:"center",gap:5,marginBottom:4}}>
                    <AlertCircle size={10}/> {removedExc.length} excursion{removedExc.length>1?"s":""} sans créneau retirée{removedExc.length>1?"s":""}
                  </div>
                )}
                ...
                {excursions.map((e,i)=>{
                  ...
                  return (
                    <button key={e.id} className={`exc-tab ${i===activeIdx?"active":""} ${p?.selectedSlot?"done":""}`}
                      ...
                      {p?.selectedSlot&&<span style={{fontSize:10,color:"rgba(52,211,153,.8)",fontWeight:700,flexShrink:0}}>{p.selectedSlot.time}</span>}
                    </button>
                  );
                })}
                {/* Excursions supprimées (grisées) */}
                {removedExc.map((e,i)=>(
                  <div key={`removed-${i}`} style={{borderRadius:12,padding:"8px 12px",background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",display:"flex",alignItems:"center",gap:8,opacity:.6}}>
                    <Trash2 size={12} color="#EF4444"/>
                    <div style={{flex:1,overflow:"hidden"}}>
                      <div style={{fontSize:11,color:"rgba(255,255,255,.5)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{sanitizeText(e.title)}</div>
                      <div style={{fontSize:10,color:"rgba(239,68,68,.7)"}}>Aucun créneau disponible</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{position:"relative",zIndex:1}}>
                ...
                {cur.hasSuggestion&&cur.suggestedDate&&(
                  <div style={{marginBottom:10}}>
                    ...
                    <button className="date-chip" onClick={()=>{
                ...
                <MiniCalendar
                  dateMap={cur.dateMap} selectedDate={cur.selectedDate}
          ...
          <div className="cmi-right">
            <div className="cmi-right-scroll">
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
                <div>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    ...
                    {cur.exc.plan_day&&(<span style={{fontSize:11,background:"#EFF9FB",color:"#2B96A8",padding:"2px 7px",borderRadius:20,fontWeight:700,display:"flex",alignItems:"center",gap:3}}><CalendarDays size={9}/>Jour {cur.exc.plan_day}</span>)}
                    {cur.exc.plan_time&&(<span style={{fontSize:11,background:"#F9FAFB",color:"#6B7280",padding:"2px 7px",borderRadius:20,fontWeight:600,display:"flex",alignItems:"center",gap:3}}>{TIME_ICON[cur.exc.plan_time]}{TIME_LABEL[cur.exc.plan_time]}</span>)}
                  </div>
                </div>
                <button onClick={()=>onClose?.()} style={{width:32,height:32,borderRadius:"50%",border:"none",background:"#F3F4F6",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#9CA3AF",flexShrink:0}}>
                  <X size={15}/>
                </button>
              </div>
              ...
              {!cur.selectedDate?(
                ...
                <div style={{marginBottom:18}}>
                  <p style={{fontSize:11,fontWeight:700,color:"#374151",marginBottom:10,display:"flex",alignItems:"center",gap:5,textTransform:"uppercase",letterSpacing:".5px"}}>
                    ...
                    {cur.selectedDate&&(<span style={{fontWeight:400,textTransform:"none",letterSpacing:0,color:"#9CA3AF"}}>— {formatDateFR(cur.selectedDate,{weekday:"long",day:"numeric",month:"long"})}</span>)}
                  </p>
              ...
              {cur.selectedSlot&&(
                <div style={{marginBottom:18}}>
                  ...
                  <p style={{fontSize:11,color:"#9CA3AF",marginTop:5}}>
                    ...
                    {cur.selectedSlot.price&&(<span style={{marginLeft:8,color:"#2B96A8",fontWeight:600}}>→ {curTotal} EUR (+{Math.round(curTotal*.1)} frais) = {curTotal+Math.round(curTotal*.1)} EUR</span>)}
                  </p>
                </div>
              )}
              ...
              <div style={{marginBottom:8}}>
                ...
                <textarea className="cmi-textarea" value={cur.specialNeeds} onChange={e=>patch(activeIdx,{specialNeeds:e.target.value})} placeholder="Allergies, mobilité réduite, préférences…"/>
              </div>
            </div>
            ...
            <div className="cmi-right-footer">
              {configuredCount>0&&configuredCount<excursions.length&&(
                <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:10,padding:"9px 12px",marginBottom:12,fontSize:12,color:"#92400E",display:"flex",alignItems:"center",gap:7}}>
                  ...
                  {excursions.length-configuredCount} excursion{excursions.length-configuredCount>1?"s":""} sans créneau — vous pouvez continuer.
                </div>
              )}
              <button
              ...
              <div style={{display:"flex",justifyContent:"center",gap:20,marginTop:12,flexWrap:"wrap"}}>
                {[
                  ...
                  <span key={g.text} style={{fontSize:11,color:"#9CA3AF",display:"flex",alignItems:"center",gap:4}}>{g.icon}{g.text}</span>
                ))}
              </div>
            </div>
          </div>
