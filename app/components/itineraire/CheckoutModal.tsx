...
interface Props {
  ...
  /**
   * true  → démarre directement à l'étape 3 (paiement),
   *         utilisé quand on arrive depuis CheckoutModalItineraire via ?pay=ID
   * false → démarre à l'étape 1 (récapitulatif normal)
   */
  autoStart?: boolean;
}
        ...
        <div className={styles["rp-modal-head"]}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: step !== 4 && !cancelled ? 20 : 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {/* Bouton retour spécial quand autoStart (on vient de l'itinéraire) */}
          ...
          {step !== 4 && !cancelled && (
            <>
              {/* Progress steps — masqués en mode autoStart (déjà payé via itinéraire) */}
              {!autoStart && (
              ...
              {/* Bandeau récap réservation en mode autoStart */}
              {autoStart && (
            ...
            <>
              ...
              {step === 3 && (
                <>
                  ...
                  <button
                    onClick={onClose}
                    style={{ width: "100%", padding: "10px", background: "transparent", border: "none", cursor: "pointer", fontSize: 12, color: "#94A3B8", textDecoration: "underline", fontFamily: "inherit" }}
                  >
