...
const MOCK = [
  { id:"1", title:"Médina de Tunis", city:"Tunis", price_per_person:45, duration_hours:3, rating:4.9, reviews_count:128, categories:["Culture","Archéologie"], photos:["https://images.unsplash.com/photo-1568515387631-8b650bbcdb90?w=600&q=80&fit=crop"], is_active:true },
  { id:"2", title:"Sidi Bou Saïd", city:"Tunis", price_per_person:35, duration_hours:2.5, rating:4.8, reviews_count:94, categories:["Culture"], photos:["https://images.unsplash.com/photo-1539020140153-e479b8c22e70?w=600&q=80&fit=crop"], is_active:true },
  { id:"3", title:"Sahara à Douz", city:"Douz", price_per_person:95, duration_hours:8, rating:5.0, reviews_count:67, categories:["Aventure","Nature"], photos:["https://images.unsplash.com/photo-1509316785289-025f5b846b35?w=600&q=80&fit=crop"], is_active:true },
  { id:"4", title:"Île de Djerba", city:"Djerba", price_per_person:55, duration_hours:4, rating:4.7, reviews_count:203, categories:["Relaxation","Nature"], photos:["https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80&fit=crop"], is_active:true },
  { id:"5", title:"Oasis de Tozeur", city:"Tozeur", price_per_person:75, duration_hours:5, rating:4.9, reviews_count:45, categories:["Nature","Aventure"], photos:["https://images.unsplash.com/photo-1548013146-72479768bada?w=600&q=80&fit=crop"], is_active:true },
  { id:"6", title:"Amphithéâtre El Jem", city:"Sfax", price_per_person:40, duration_hours:4, rating:4.8, reviews_count:112, categories:["Archéologie","Culture"], photos:["https://images.unsplash.com/photo-1568515387631-8b650bbcdb90?w=600&q=80&fit=crop"], is_active:true },
];

export default function ExcursionsPage() {
  const [excursions, setExcursions] = useState<typeof MOCK>([]);
  const [filtered, setFiltered] = useState<typeof MOCK>([]);
  ...
  useEffect(() => {
    ...
    supabase.from("excursions").select("*").eq("is_active", true)
      .then(({ data }) => {
        setExcursions(data?.length ? data as typeof MOCK : MOCK);
      });
  }, []);
      ...
      <div style={{ background:"#FAFAF9", minHeight:"100vh", padding:"40px 52px" }}>
        <div style={{ maxWidth:1160, margin:"0 auto" }}>
          ...
          <div style={{ marginBottom:32 }}>
            ...
            <p style={{ color:"#6B7280", fontSize:15 }}>{filtered.length} excursion{filtered.length > 1 ? "s" : ""} disponible{filtered.length > 1 ? "s" : ""}</p>
          </div>
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign:"center", padding:"80px 20px" }}>
              ...
              <p style={{ fontSize:18, fontWeight:600, color:"#374151", marginTop:16 }}>Aucune excursion trouvée</p>
              <p style={{ color:"#9CA3AF", marginTop:8 }}>Essayez d&apos;autres filtres</p>
            </div>
          ) : (
              ...
              {filtered.map((exc) => (
                <div key={exc.id} className="exc-card" onClick={() => window.location.href=`/excursions/${exc.id}`}>
                  <div style={{ position:"relative", height:210, overflow:"hidden" }}>
                    <img src={exc.photos?.[0] || "https://images.unsplash.com/photo-1568515387631-8b650bbcdb90?w=600&q=80"} alt={exc.title}
                      style={{ width:"100%", height:"100%", objectFit:"cover" }} />
                    {/* Heart */}
                    <button className="heart" onClick={() => toggleFav(exc.id)}
                  ...
                  <div style={{ padding:16 }}>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12 }}>
                      ...
                      <button onClick={() => { if (!user) { sessionStorage.setItem("redirect_after_login", "/excursions"); window.location.href = "/auth"; } }}
          ...
          {!user && (
            <div style={{ marginTop:48, padding:"28px 36px", background:"linear-gradient(135deg,#EFF9FB,#F0FFFE)", border:"1.5px solid #B2E3EB", borderRadius:18, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                <p style={{ fontSize:14, color:"#6B7280" }}>Les ❤️ favoris, réservations et paiements nécessitent un compte gratuit</p>
              </div>
