  ...
  const sendMessage = async () => {
    ...
    if (!convId) {
      const { data: newConv } = await supabase.from("conversations").insert({
        ...
        touriste_name: user.full_name, prestataire_name: prestataire.agency_name || prestataire.full_name,
      }).select().single();
      convId = newConv?.id;
    }
    if (convId) {
        ...
        <div style={{ marginBottom:22 }}>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:18,flexWrap:"wrap" }}>
            <span style={{ display:"flex",alignItems:"center",gap:5,fontSize:13,color:"#6B7280",fontWeight:600 }}>
              <MapPin size={13} color="#2B96A8"/>{exc.city}
            </span>
            {avgRating && (
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 340px",gap:28 }}>
          ...
          <div>
            ...
            <div style={{ background:"white",borderRadius:20,border:"1px solid #EBEBEB",padding:"24px",marginBottom:16 }}>
              ...
              <p style={{ fontSize:14.5,color:"#374151",lineHeight:1.9,position:"relative" }}>
                {LONG_DESC && !descExpanded
                  ? <>{exc.description.slice(0, 380)}<span style={{ color:"#9CA3AF" }}>…</span></>
                  : exc.description
                }
              </p>
              {LONG_DESC && (
                        ...
                        <div style={{ marginLeft:52,padding:"10px 14px",background:"rgba(43,150,168,.06)",borderRadius:10,borderLeft:"3px solid #2B96A8",marginBottom:12 }}>
                          <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:5 }}>
                            <div style={{ width:24,height:24,borderRadius:"50%",overflow:"hidden",flexShrink:0,background:"linear-gradient(135deg,#2B96A8,#1e7a8a)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:10,fontWeight:800 }}>
                              {prestataire?.avatar_url ? <img src={prestataire.avatar_url} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }} /> : prestName[0].toUpperCase()}
                            </div>
            ...
            <div style={{ background:"white",borderRadius:24,border:"1px solid #EBEBEB",padding:"26px",boxShadow:"0 12px 48px rgba(0,0,0,.1)" }}>
              ...
              <div style={{ background:"#F9FAFB",borderRadius:14,padding:"13px 14px",marginBottom:18 }}>
                {[
                  ...
                  { icon:<MapPin size={13} color="#6B7280"/>, label:"Lieu",     val:exc.city },
                  { icon:<Tag size={13} color="#6B7280"/>,    label:"Catégorie",val:exc.categories?.[0]||"—" },
                ].map((r, i) => (
                    ...
                    <div style={{ display:"flex",alignItems:"center",gap:12 }}>
                      <div style={{ width:44,height:44,borderRadius:"50%",overflow:"hidden",flexShrink:0,border:"2px solid #F0F0F0" }}>
                        {prestataire.avatar_url
                          ...
                          : <div style={{ width:"100%",height:"100%",background:"linear-gradient(135deg,#2B96A8,#1e7a8a)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:800,fontSize:17 }}>{prestName[0].toUpperCase()}</div>
                        }
                      </div>
                      <div style={{ flex:1,minWidth:0 }}>
                        <p style={{ fontSize:13,fontWeight:800,color:"#111827",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{prestName}</p>
                        {prestataire.city && <p style={{ fontSize:11,color:"#9CA3AF",marginTop:1,display:"flex",alignItems:"center",gap:3 }}><MapPin size={9}/>{prestataire.city}</p>}
        ...
        <div className="overlay" onClick={e => { if(e.target===e.currentTarget) setShowCheckout(false); }}>
          <div className="modal fu">
            ...
            <div style={{ background:"#F9FAFB",borderRadius:14,padding:"14px 16px",marginBottom:22 }}>
              <p style={{ fontSize:15,fontWeight:700,color:"#111827",marginBottom:4 }}>{exc.title}</p>
              <p style={{ fontSize:12,color:"#6B7280",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap" }}>
                <span style={{ display:"flex",alignItems:"center",gap:4 }}><MapPin size={11}/>{exc.city}</span>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:16,marginBottom:22 }}>
              {/* Date */}
              <div>
                <label style={{ display:"block",fontSize:11,fontWeight:700,color:"#374151",letterSpacing:.5,textTransform:"uppercase",marginBottom:7,display:"flex",alignItems:"center",gap:5 }}>
          ...
          <div className="modal fu">
            ...
            <div style={{ display:"flex",alignItems:"center",gap:18,marginBottom:20,paddingBottom:18,borderBottom:"1px solid #F3F4F6" }}>
              <div style={{ width:76,height:76,borderRadius:"50%",overflow:"hidden",flexShrink:0,border:"3px solid #F0F0F0",boxShadow:"0 4px 16px rgba(0,0,0,.08)" }}>
                {prestataire.avatar_url
                  ? <img src={prestataire.avatar_url} alt={prestName} style={{ width:"100%",height:"100%",objectFit:"cover" }} />
                  : <div style={{ width:"100%",height:"100%",background:"linear-gradient(135deg,#2B96A8,#4AABB8)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:800,fontSize:28 }}>{prestName[0].toUpperCase()}</div>
                }
              </div>
              <div>
                ...
                {prestataire.full_name && prestataire.agency_name && (
                  <p style={{ fontSize:13,color:"#6B7280",marginBottom:6,display:"flex",alignItems:"center",gap:5 }}><Users size={12}/>{prestataire.full_name}</p>
                )}
                <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
                  ...
                  {prestataire.city && (
                    <span style={{ padding:"3px 10px",background:"#F3F4F6",color:"#374151",borderRadius:20,fontSize:11,fontWeight:600,display:"flex",alignItems:"center",gap:4 }}>
                      <MapPin size={10}/>{prestataire.city}
                    </span>
                  )}
                </div>
              </div>
