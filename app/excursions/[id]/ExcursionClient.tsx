...
function deriveCatStyle(couleur: string): { text: string; bg: string; border: string } {
...
export default function ExcursionClient({
  exc, prestataire, initialAvis, myLikedIds = [], categories = [],
}: {
  exc: Excursion;
          ...
          <div className="exc-title-inner">
            <div className="exc-categories">
              {exc.categories?.map(c => {
                const cs = catStyle(c);
                return (
                  <span key={c} className="exc-category" style={{ background: cs.bg, color: cs.text, border: `1px solid ${cs.border}` }}>
                    {sanitizeText(c)}
                  </span>
                );
              })}
            ...
            <div className="exc-meta">
              ...
              {avgRating && (
                <span className="exc-rating-badge">
                  ...
                  <span className="exc-rating-count">({avis.length} avis)</span>
                </span>
              )}
            </div>
          </div>
        ...
        <div className="exc-grid">
          ...
          <div className="exc-left">
            ...
            <div className="glass-card glass-card-padded">
              ...
              {LONG_DESC && (
                <button onClick={() => setDescExpanded(p => !p)} className="exc-desc-expand-btn">
              ...
              {user && !alreadyReviewed && !avisSuccess && (
                <div className="glass-card review-form">
                  <div className="review-form-header">
                    ...
                    <h3 className="review-form-title">Partagez votre expérience</h3>
                  </div>
                  {avisError && (
                  ...
                  <form onSubmit={submitAvis}>
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12 }}>
                      <button
                        ...
                        className={myRating > 0 ? "review-submit-btn review-submit-btn-active" : "review-submit-btn review-submit-btn-disabled"}
                      >
                      ...
                      <div className="review-header">
                        <div className="review-user">
                          <div className="review-avatar">
                            {a.touriste_avatar
                              ? <img src={a.touriste_avatar} alt="" className="review-avatar-img" />
                              : (a.touriste_name?.[0] || "T").toUpperCase()
                            }
                          </div>
                          <div>
                            <p className="review-name">{sanitizeText(a.touriste_name || "Voyageur")}</p>
                      ...
                      {a.prestataire_response && (
                        <div className="review-response">
                          <div className="review-response-header">
                            ...
                            <p className="review-response-name">{sanitizeText(prestName)}</p>
                            <span className="review-response-badge">· Guide</span>
                          </div>
                          <p className="review-response-text">{sanitizeText(a.prestataire_response)}</p>
                        </div>
                      )}
            ...
            <div className="glass-card booking-card">
              <div className="booking-price">
                <div className="booking-price-main">
                  <span className="booking-price-value">{exc.price_per_person}</span>
                  <div>
              ...
              <div className="booking-details">
                {[
                  ...
                  <div key={r.label} className="booking-detail-row" style={{ borderBottom: i < 3 ? "1px solid var(--border)" : "none" }}>
                    <span className="booking-detail-label">{r.icon}{r.label}</span>
              ...
              <div style={{ height: 8 }} />
              {prestataire && (
                <button className="btn-msg" onClick={() => { if (!user) { sessionStorage.setItem("redirect_after_login", `/excursions/${exc.id}`); window.location.href = "/auth"; return; } setShowMsgModal(true); }}>
              ...
              <div className="booking-guarantees">
                {[
                  ...
                  <div key={g.text} className="guarantee-item" style={{ background: g.bg }}>
                    {g.icon}
                ...
                <div className="guide-card" onClick={() => setShowPrestataire(true)}>
                  <div className="guide-header">
                    <div className="guide-avatar">
                      {prestataire.avatar_url
                        ? <img src={prestataire.avatar_url} alt={sanitizeText(prestName)} className="guide-avatar-img" />
                        : <div className="guide-avatar-placeholder">{sanitizeText(prestName)[0].toUpperCase()}</div>
                      }
                    </div>
                    <div className="guide-info">
                      <p className="guide-name">{sanitizeText(prestName)}</p>
                      {prestataire.city && <p className="guide-city"><MapPin size={9} />{sanitizeText(prestataire.city)}</p>}
                      <p className="guide-verified"><ShieldCheck size={11} />Vérifié</p>
                    </div>
              ...
              <>
                <div className="modal-header">
                  <div>
                    ...
                    <p className="modal-subtitle">À propos de : {sanitizeText(exc.title)}</p>
                  </div>
                  <button onClick={() => { setShowMsgModal(false); setMsgText(""); }} className="modal-close">
                    <X size={15} />
                  </button>
                </div>
                {prestataire && (
                  <div className="modal-guide-preview">
                    <div className="modal-guide-avatar">
                      {prestataire.avatar_url
                        ? <img src={prestataire.avatar_url} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" />
                        : sanitizeText(prestName)[0].toUpperCase()
                      }
                    </div>
                    <div>
                ...
                <div className="modal-actions">
                  <button onClick={() => { setShowMsgModal(false); setMsgText(""); }} className="modal-cancel-btn">
                    Annuler
                  </button>
                  <button
                    onClick={sendMessage}
                    disabled={!msgText.trim() || msgSending}
                    className={msgText.trim() ? "modal-send-btn modal-send-btn-active" : "modal-send-btn modal-send-btn-disabled"}
                  >
        ...
        <div className="overlay" onClick={e => { if (e.target === e.currentTarget) setShowPrestataire(false); }}>
          <div className="modal fu">
            <div className="prestataire-modal-header">
              ...
              <button onClick={() => setShowPrestataire(false)} className="modal-close">
                <X size={16} />
              </button>
            </div>
            <div className="prestataire-profile">
              ...
              <div className="prestataire-info">
                <h3 className="prestataire-name">{sanitizeText(prestName)}</h3>
                {prestataire.full_name && prestataire.agency_name && (
                  <p className="prestataire-fullname"><Users size={12} />{sanitizeText(prestataire.full_name)}</p>
                )}
