...
interface Prestataire {
  ...
  is_validated: boolean; avatar_url: string | null; email?: string;
  created_at: string;
}
interface Avis {
...
export default function AdminExcursionDetail({
  ...
  const toggleLike = async (avisId: string) => {
    ...
    setMyLikes(prev => {
      ...
      liked ? next.delete(avisId) : next.add(avisId);
      return next;
    });
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .abtn{padding:7px 13px;border-radius:9px;border:1px solid #E5E7EB;cursor:pointer;font-size:12px;font-weight:700;font-family:inherit;transition:all .2s;background:white}
        ...
        .toast-ad{position:fixed;top:22px;right:22px;z-index:9999;padding:12px 20px;border-radius:14px;font-size:14px;font-weight:600;font-family:inherit;box-shadow:0 8px 28px rgba(0,0,0,.12);animation:tin .3s ease}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 22, fontSize: 13, color: "#9CA3AF" }}>
        <Link href="/admin/excursions" style={{ color: "#6B7280", textDecoration: "none", fontWeight: 500 }}>← Excursions</Link>
        <span>/</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 310px", gap: 28 }}>
        ...
        <div>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 900, color: "#111827", marginBottom: 4, letterSpacing: "-.5px" }}>
              {exc.title}
            </h1>
            <p style={{ fontSize: 13, color: "#6B7280" }}>📍 {exc.city} · créée le {new Date(exc.created_at).toLocaleDateString("fr-FR")}</p>
          </div>
          ...
          <div style={{ borderRadius: 18, overflow: "hidden", aspectRatio: "16/9", background: "#F3F4F6", position: "relative", marginBottom: 10 }}>
            ...
            {photos.length > 1 && (
              <>
                ...
                <div style={{ position: "absolute", bottom: 10, right: 12, padding: "3px 9px", background: "rgba(0,0,0,.5)", backdropFilter: "blur(6px)", borderRadius: 20, fontSize: 11, color: "white", fontWeight: 700 }}>
              ...
              {exc.inclusions?.length > 0 && (
                <div>
                  ...
                  {exc.inclusions.map(inc => (
                    <div key={inc} style={{ display: "flex", gap: 7, fontSize: 13, color: "#374151", marginBottom: 5 }}>
                      <span style={{ color: "#15803D", fontWeight: 800 }}>✓</span>{inc}
                    </div>
                  ))}
                </div>
              )}
              {exc.languages?.length > 0 && (
                <div>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {exc.languages.map(l => (
                      <span key={l} style={{ padding: "4px 12px", background: "#F3F4F6", borderRadius: 20, fontSize: 12, fontWeight: 600, color: "#374151" }}>🌍 {l}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div style={{ marginTop: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
              <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontWeight: 900, color: "#111827" }}>
                Avis {avgRating && <span style={{ fontSize: 16, color: "#F59E0B" }}>★ {avgRating}</span>}
              </h2>
              <div style={{ display: "flex", gap: 7, fontSize: 12 }}>
                <span style={{ padding: "3px 10px", background: "#F0FDF4", color: "#15803D", borderRadius: 20, fontWeight: 700 }}>{approvedAvis.length} publiés</span>
                {avis.filter(a => !a.is_moderated).length > 0 && (
                  <span style={{ padding: "3px 10px", background: "#FFFBEB", color: "#D97706", borderRadius: 20, fontWeight: 700 }}>
                    {avis.filter(a => !a.is_moderated).length} en attente
                  </span>
                )}
              </div>
            </div>
                ...
                <div key={a.id} className="av-card fu"
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 1 }}>
                      {STARS.map(s => <span key={s} style={{ fontSize: 15, color: s <= a.rating ? "#F59E0B" : "#E5E7EB" }}>★</span>)}
                    </div>
                  </div>

                  {/* Commentaire */}
                  {a.comment && (
                    <div style={{ background: "#F9FAFB", borderRadius: 10, padding: "10px 14px", paddingLeft: 50, marginBottom: 12 }}>
                  ...
                  {a.prestataire_response && (
                    <div style={{ marginLeft: 48, padding: "10px 14px", background: "rgba(43,150,168,.06)", borderRadius: 10, borderLeft: "3px solid #2B96A8", marginBottom: 10 }}>
                      <p style={{ fontSize: 11, fontWeight: 800, color: "#2B96A8", marginBottom: 4 }}>🏢 Réponse du prestataire</p>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 10, borderTop: "1px solid #F3F4F6", marginTop: 6 }}>
                    {/* Like */}
                    <button className={`like-btn ${myLikes.has(a.id) ? "on" : ""}`}
                      onClick={() => toggleLike(a.id)}>
                      <span>{myLikes.has(a.id) ? "❤️" : "🤍"}</span>
                    ...
                    {/* Approuver */}
                    {!a.is_moderated && (
                      <button className="abtn" disabled={loadingId === a.id}
                        onClick={() => approveAvis(a.id)}
                        style={{ color: "#15803D", borderColor: "#BBF7D0", background: "#F0FDF4" }}>
                        {loadingId === a.id ? "..." : "✅ Approuver"}
                      </button>
                    )}
        ...
        <div style={{ position: "sticky", top: 80, height: "fit-content" }}>
          ...
          <div style={{ background: "white", borderRadius: 18, border: "1px solid #F0F0F0", padding: "20px", marginBottom: 14, boxShadow: "0 2px 12px rgba(0,0,0,.06)" }}>
            <h3 style={{ fontSize: 13, fontWeight: 800, color: "#111827", marginBottom: 14, paddingBottom: 10, borderBottom: "1px solid #F3F4F6" }}>📊 Statistiques</h3>
            {[
              { label: "Prix / pers.", val: `${exc.price_per_person} TND`, icon: "💰" },
              { label: "Durée", val: `${exc.duration_hours}h`, icon: "⏱️" },
              { label: "Capacité", val: `${exc.max_people} pers.`, icon: "👥" },
              { label: "Note moy.", val: avgRating ? `${avgRating}/5` : "—", icon: "⭐" },
              { label: "Réservations", val: `${reservations.length}`, icon: "🗓️" },
              { label: "Confirmées", val: `${reservations.filter(r => r.status === "confirmed").length}`, icon: "✅" },
            ].map(s => (
            ...
            <div
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {prestataire.city && <p style={{ fontSize: 12, color: "#9CA3AF" }}>📍 {prestataire.city}</p>}
                  <span style={{ fontSize: 11, fontWeight: 700, color: prestataire.is_validated ? "#15803D" : "#D97706", background: prestataire.is_validated ? "#F0FDF4" : "#FFFBEB", padding: "2px 8px", borderRadius: 8, display: "inline-block", marginTop: 3 }}>
          ...
          <div style={{ background: "white", borderRadius: 18, border: "1px solid #F0F0F0", padding: "18px 20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <a href={`/excursions/${exc.id}`} target="_blank" rel="noopener noreferrer"
                style={{ padding: "10px 14px", background: "#F9FAFB", color: "#374151", border: "1px solid #E5E7EB", borderRadius: 12, textDecoration: "none", fontSize: 13, fontWeight: 700, textAlign: "center" }}>
                👁️ Page publique
              </a>
              <Link href="/admin/excursions"
                style={{ padding: "10px 14px", background: "white", color: "#6B7280", border: "1px solid #E5E7EB", borderRadius: 12, textDecoration: "none", fontSize: 13, fontWeight: 700, textAlign: "center" }}>
                ← Retour
              </Link>
            </div>
          </div>
          ...
          <div className="modal-box fu">
            ...
            <div style={{ padding: "24px 28px", borderBottom: "1px solid #F3F4F6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setShowPrestataire(false)}
                style={{ background: "#F3F4F6", border: "none", borderRadius: "50%", width: 32, height: 32, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>
                ✕
              </button>
            </div>
            ...
            <div style={{ padding: "24px 28px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
                {[
                  { icon: "📍", label: "Ville", val: prestataire.city || "—" },
                  { icon: "📞", label: "Téléphone", val: prestataire.phone || "—" },
                  { icon: "🗓️", label: "Inscrit le", val: new Date(prestataire.created_at).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" }) },
                ].map(row => (
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <Link href={`/admin/prestataires`}
                  ...
                  style={{ flex: 1, padding: "11px 14px", background: "rgba(43,150,168,.08)", color: "#2B96A8", border: "1.5px solid rgba(43,150,168,.2)", borderRadius: 12, textDecoration: "none", fontSize: 13, fontWeight: 700, textAlign: "center" }}>
                  Gérer les prestataires →
                </Link>
                <button onClick={() => setShowPrestataire(false)}
                  style={{ padding: "11px 18px", background: "#F3F4F6", color: "#374151", border: "none", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                  Fermer
                </button>
              </div>
            </div>
