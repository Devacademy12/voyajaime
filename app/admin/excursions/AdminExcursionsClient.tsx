...
export default function AdminExcursionsClient({ excursions: initial }: { excursions: Excursion[] }) {
  ...
  const toggleActive = async (id: string, currentIsActive: boolean) => {
    ...
    // Optimistic update
    setExcursions(prev =>
      prev.map(e => e.id === id ? { ...e, is_active: !currentIsActive } : e)
    );
    ...
    try {
      const res = await fetch("/api/admin/manage-excursion", {
        ...
        body: JSON.stringify({
          action: "toggle",
          id,
          value: { is_active: !currentIsActive },
        }),
      });
      ...
      showToast(
        !currentIsActive ? "Excursion publiée ✓" : "Excursion dépubliée ✓",
        true
      );
    } catch (e) {
      // Rollback optimistic update on error
      setExcursions(prev =>
        prev.map(ex => ex.id === id ? { ...ex, is_active: currentIsActive } : ex)
      );
  ...
  const filtered = excursions.filter(e => {
    ...
    const matchSearch =
      ...
      e.prestataire_name.toLowerCase().includes(q);
      ...
      // ── Grid view ──────────────────────────────────────────────────────────
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 18 }}>
          {filtered.map(exc => (
            <div
              key={exc.id}
              className="admin-exc-card"
              style={{ cursor: "pointer" }}
              onClick={e => {
                if ((e.target as HTMLElement).closest("button,a")) return;
                window.location.href = `/admin/excursions/${exc.id}`;
              }}
            >
              {/* Image */}
              <div style={{ overflow: "hidden", position: "relative", height: 200, background: "#F3F4F6" }}>
                ...
                <div style={{ position: "absolute", top: 10, left: 10 }}>
                  <span style={{
                    ...
                    background: exc.is_active ? "rgba(21,128,61,.88)" : "rgba(107,114,128,.82)",
              ...
              <div style={{ padding: "16px 18px" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    className="admin-exc-btn"
                    disabled={loading === exc.id}
                    onClick={e => { e.stopPropagation(); toggleActive(exc.id, exc.is_active); }}
                    style={{
                      flex: 1, justifyContent: "center",
                      color:       exc.is_active ? "#374151"  : "#059669",
                      background:  exc.is_active ? "white"    : "#F0FDF4",
                      borderColor: exc.is_active ? "#E5E7EB"  : "#BBF7D0",
                    }}
                  >
                    {loading === exc.id
            ...
            >
              ...
              <div className="admin-exc-row-actions">
                <button
                  ...
                  style={{
                    padding: "6px 10px",
                    background:  exc.is_active ? "#FEF2F2"  : "#F0FDF4",
                    color:       exc.is_active ? "#DC2626"  : "#059669",
                    borderColor: exc.is_active ? "#FECACA"  : "#BBF7D0",
                  }}
                >
                  {loading === exc.id
