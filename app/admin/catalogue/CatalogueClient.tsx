...
export default function CatalogueClient({
  ...
  return (
    <>
      <style>{`
        .ctab{padding:9px 20px;border-radius:20px;border:1px solid #E5E7EB;cursor:pointer;font-size:13px;font-weight:600;font-family:inherit;transition:all .2s;display:inline-flex;align-items:center;gap:7px}
        ...
        .cbtn{padding:7px 12px;border-radius:9px;border:1px solid #E5E7EB;cursor:pointer;font-size:12px;font-weight:700;font-family:inherit;transition:all .2s;background:white;white-space:nowrap;display:inline-flex;align-items:center;gap:5px}
      ...
      <div style={{ display:"flex", gap:8, marginBottom:24 }}>
        <button className={`ctab ${tab==="villes"?"on":""}`} onClick={()=>setTab("villes")}>
          <MapPin size={14} strokeWidth={1.5} /> Villes
          <span style={{ fontSize:11, fontWeight:800, padding:"1px 7px", borderRadius:12, background: tab==="villes"?"rgba(255,255,255,.25)":"#F3F4F6", color: tab==="villes"?"white":"#6B7280" }}>{villes.length}</span>
        </button>
        <button className={`ctab ${tab==="categories"?"on":""}`} onClick={()=>setTab("categories")}>
          <Tag size={14} strokeWidth={1.5} /> Catégories
          <span style={{ fontSize:11, fontWeight:800, padding:"1px 7px", borderRadius:12, background: tab==="categories"?"rgba(255,255,255,.25)":"#F3F4F6", color: tab==="categories"?"white":"#6B7280" }}>{categories.length}</span>
        </button>
      </div>
        ...
        <div>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            ...
            <button className="cbtn" onClick={()=>setVilleModal({...DEFAULT_VILLE})}
              style={{ background:"#2B96A8", color:"white", borderColor:"#2B96A8", padding:"9px 16px", fontSize:13 }}>
              ...
              <div key={v.id} className="ccard" style={{ borderLeft:`3px solid ${v.active?"#2B96A8":"#E5E7EB"}` }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:2 }}>
                    ...
                    <span style={{ fontSize:11, fontWeight:700, padding:"1px 7px", borderRadius:10, background:v.active?"#F0FDF4":"#F9FAFB", color:v.active?"#15803D":"#9CA3AF", display:"inline-flex", alignItems:"center", gap:3 }}>
                      {v.active ? <CheckCircle size={9} /> : <Pause size={9} />}
                ...
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  <button className="cbtn" disabled={villeLoading===v.id} onClick={()=>toggleVille(v)}
                    ...
                    {villeLoading===v.id ? "..." : v.active ? <Pause size={13} /> : <Play size={13} />}
                  </button>
                  <button className="cbtn" disabled={villeLoading===v.id} onClick={()=>setVilleModal({...v})}
                    style={{ color:"#2B96A8", borderColor:"rgba(43,150,168,.3)", background:"rgba(43,150,168,.05)" }}>
                    <Pencil size={13} />
                  </button>
                  <button className="cbtn" disabled={villeLoading===v.id} onClick={()=>deleteVille(v.id,v.nom)}
          ...
          {villes.length === 0 && (
            <div style={{ textAlign:"center", padding:"48px", background:"white", borderRadius:16, border:"1px solid #F3F4F6" }}>
              <MapPin size={36} color="#D1D5DB" strokeWidth={1.5} style={{ marginBottom:12 }} />
      ...
      {tab === "categories" && (
        <div>
              ...
              <div key={c.id} className="ccard" style={{ borderLeft:`3px solid ${c.couleur}` }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                    ...
                    <span style={{ width:10, height:10, borderRadius:"50%", background:c.couleur, display:"inline-block", flexShrink:0 }}/>
                  </div>
                  <p style={{ fontSize:11, color:"#9CA3AF", fontFamily:"monospace" }}>{c.couleur}</p>
                </div>
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  <button className="cbtn" disabled={catLoading===c.id} onClick={()=>setCatModal({...c})}
                    style={{ color:"#2B96A8", borderColor:"rgba(43,150,168,.3)", background:"rgba(43,150,168,.05)" }}>
                    <Pencil size={13} />
                  </button>
                  <button className="cbtn" disabled={catLoading===c.id} onClick={()=>deleteCat(c.id,c.nom)}
                    style={{ color:"#DC2626", borderColor:"#FECACA", background:"#FEF2F2" }}>
                    <Trash2 size={13} />
                  </button>
                </div>
          ...
          {categories.length === 0 && (
            <div style={{ textAlign:"center", padding:"48px", background:"white", borderRadius:16, border:"1px solid #F3F4F6" }}>
              <Tag size={36} color="#D1D5DB" strokeWidth={1.5} style={{ marginBottom:12 }} />
        ...
        <div className="overlay" onClick={e=>{if(e.target===e.currentTarget)setVilleModal(null)}}>
          <div className="modal">
            <div style={{ padding:"22px 26px 0", display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
              ...
              <button onClick={()=>setVilleModal(null)} style={{ background:"#F3F4F6", border:"none", borderRadius:"50%", width:30, height:30, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <X size={15} />
              </button>
            </div>
            <div style={{ padding:"0 26px 26px", display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div style={{ display:"flex", gap:8, marginTop:4 }}>
                <button onClick={()=>setVilleModal(null)} style={{ flex:1, padding:"11px", background:"#F3F4F6", color:"#374151", border:"none", borderRadius:12, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                  <X size={14} /> Annuler
                </button>
                <button onClick={saveVille} disabled={villeLoading==="save"}
                  style={{ flex:2, padding:"11px", background:villeLoading?"#9CA3AF":"#2B96A8", color:"white", border:"none", borderRadius:12, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6, transition:"all .2s" }}>
                  {villeLoading==="save" ? "Sauvegarde..." : villeModal.id ? <><Save size={14}/> Enregistrer</> : <><Plus size={14}/> Ajouter</>}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="overlay" onClick={e=>{if(e.target===e.currentTarget)setCatModal(null)}}>
          <div className="modal">
            <div style={{ padding:"22px 26px 0", display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
              ...
              <button onClick={()=>setCatModal(null)} style={{ background:"#F3F4F6", border:"none", borderRadius:"50%", width:30, height:30, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <X size={15} />
              </button>
            </div>
            <div style={{ padding:"0 26px 26px", display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div>
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                  {COULEURS_PRESET.map(col=>(
                    <button key={col} type="button" className={`color-swatch ${catModal.couleur===col?"on":""}`} style={{ background:col }} onClick={()=>setCatModal(p=>({...p!,couleur:col}))} />
                  ))}
              ...
              <div style={{ display:"flex", gap:8, marginTop:4 }}>
                <button onClick={()=>setCatModal(null)} style={{ flex:1, padding:"11px", background:"#F3F4F6", color:"#374151", border:"none", borderRadius:12, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                  <X size={14} /> Annuler
                </button>
                <button onClick={saveCat} disabled={catLoading==="save"}
                  style={{ flex:2, padding:"11px", background:catLoading?"#9CA3AF":"#2B96A8", color:"white", border:"none", borderRadius:12, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6, transition:"all .2s" }}>
                  {catLoading==="save" ? "Sauvegarde..." : catModal.id ? <><Save size={14}/> Enregistrer</> : <><Plus size={14}/> Ajouter</>}
                </button>
              </div>
            </div>
          </div>
