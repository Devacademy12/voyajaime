...
export default async function AdminDashboard() {
  ...
  const { data: resaRaw } = await supabase
    .from("reservations")
    .select("created_at, booking_mode")
  ...
  for (let i = 0; i < 30; i++) {
    ...
    resaByDay[key] = { total: 0, assiste: 0 };
  }
  (resaRaw || []).forEach(r => {
    const key = String(r.created_at).split("T")[0];
    if (resaByDay[key]) {
      resaByDay[key].total++;
      if (r.booking_mode === "assisted") resaByDay[key].assiste++;
    }
  });
  const reservationsParJour = Object.entries(resaByDay).map(([date, v]) => ({
    date,
    total: v.total,
    assiste: v.assiste,
  }));
  ...
  const { data: villesRaw } = await supabase
    .from("reservations")
    .select("excursion:excursions(city)");

  const villeCount: Record<string, number> = {};
  (villesRaw || []).forEach(r => {
    const exc  = r.excursion as { city?: string } | null;
    const city = exc?.city;
    if (city) villeCount[city] = (villeCount[city] || 0) + 1;
  });
  ...
  // ── Stats visiteurs (remplace par Plausible/PostHog/Vercel Analytics) ──────
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',system-ui,sans-serif" }}>
      <style>{`
        ...
        .d-charts-mid { display:grid; grid-template-columns:1fr 1fr; gap:16px; }
        @media(max-width:1024px) { .d-kpi { grid-template-columns:repeat(2,1fr); } }
        @media(max-width:700px) {
          .d-mid { grid-template-columns:1fr; }
          .d-charts-mid { grid-template-columns:1fr; }
        }
      ...
      <div className="dp-card" style={{ marginBottom: 28, animationDelay: "0s" }}>
        ...
        <p style={{ color: "#6B7280", fontSize: 14, margin: "4px 0 0" }}>
          Vue globale de la plateforme VoyajAime
        </p>
      </div>
      ...
      <div className="d-kpi dp-card" style={{ animationDelay: ".07s" }}>
        {[
          ...
          >
            ...
            <p style={{ fontSize: 26, fontWeight: 900, color: "#053366", lineHeight: 1, margin: 0 }}>
              {s.value}
            </p>
            <p style={{ fontSize: 12, color: "#9CA3AF", fontWeight: 500, margin: 0, textTransform: "uppercase", letterSpacing: "0.5px" }}>
              {s.label}
            </p>
          </div>
        ))}
        ...
        {[
          {
            ...
            value:  `${totalRevenue.toLocaleString("fr-FR")} TND`,
            Icon:   TrendingUp,
            color:  "#02AFCF",
            border: "#02AFCF",
            bg:     "rgba(2,175,207,.1)",
          ...
          {
            label:  "Commission encaissée (10%)",
            value:  `${totalFees.toLocaleString("fr-FR")} TND`,
            Icon:   Coins,
            color:  "#259FFC",
            border: "#259FFC",
            bg:     "rgba(37,159,252,.1)",
          ...
          >
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 12, color: "#9CA3AF", fontWeight: 500, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.4px" }}>
                {s.label}
              </p>
              <p style={{ fontSize: 24, fontWeight: 900, color: "#053366", lineHeight: 1, margin: 0 }}>
                {s.value}
              </p>
            </div>
          </div>
      ...
      {pendingPrestataires && pendingPrestataires.length > 0 && (
        <div
          ...
          style={{
            marginBottom: 20,
            padding: "14px 18px",
            background: "linear-gradient(135deg,rgba(2,175,207,.07),rgba(37,159,252,.05))",
            border: "1px solid rgba(2,175,207,.25)",
            borderRadius: 14,
            display: "flex",
        ...
        >
          <p style={{ fontSize: 14, color: "#053366", fontWeight: 600, display: "flex", alignItems: "center", gap: 8, margin: 0 }}>
            <Clock size={16} color="#02AFCF" strokeWidth={1.5} />
            <span>
              <strong style={{ color: "#02AFCF" }}>{pendingPrestataires.length} prestataire(s)</strong>{" "}
              en attente de validation
            </span>
          </p>
          
           <a href="/admin/prestataires"
            style={{ fontSize: 13, color: "#259FFC", fontWeight: 700, textDecoration: "none", display: "flex", alignItems: "center", gap: 4, whiteSpace: "nowrap" }}> Valider < ChevronRight size={14} />
          </a>
        </div>
      )}

      {/* ── Prestataires à valider + Avis à modérer ── */}
              ...
              {pendingPrestataires.slice(0, 3).map(p => (
                ...
                >
                  <div style={{ minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 12, color: "#9CA3AF", display: "flex", alignItems: "center", gap: 4, marginTop: 3, margin: "3px 0 0" }}>
                   ...
                   <a href="/admin/prestataires"
                    style={{
                      ...
                      gap: 4,
                      flexShrink: 0,
                      boxShadow: "0 2px 8px rgba(2,175,207,.3)",
                    }}
                  >
            ...
            <div>
              {pendingAvis.map(a => {
                ...
                return (
                  <div
                    key={String(a.id)}
                    style={{ padding: "10px 14px", background: "#F8FAFF", borderRadius: 12, marginBottom: 8, border: "1px solid #EEF2FF" }}
                  >
      ...
      <div className="dp-card" style={{ animationDelay: ".32s" }}>
        <AdminCharts
