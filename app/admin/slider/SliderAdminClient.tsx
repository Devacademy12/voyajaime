...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ── Type tabs ── */
  .type-tab{display:flex;align-items:center;gap:6px;padding:9px 16px;border-radius:10px;border:1.5px solid #E5E7EB;background:white;font-size:13px;font-weight:600;color:#374151;cursor:pointer;font-family:inherit;transition:all .18s;white-space:nowrap}
  ...
  @media (max-width: 600px) {
    ...
    /* Card actions: condense on small screens */
    .slide-card-actions { gap: 4px !important; }
    .slide-card-actions .action-btn { width: 28px !important; height: 28px !important; }
    .slide-card-actions .move-btns  { display: none !important; }

    /* Add form */
    .add-form-type-tabs { gap: 6px !important; overflow-x: auto; padding-bottom: 2px; }
    .add-form-type-tabs::-webkit-scrollbar { display: none; }
    .add-form-type-tabs .type-tab { font-size: 12px !important; padding: 7px 12px !important; }
...
export default function SliderAdminClient({
  ...
  // detect mobile (for sheet vs sidebar)
  const [isMobile, setIsMobile] = useState(false);
  if (typeof window !== "undefined") {
    // only set once on client
  }
  // use effect equivalent via inline check — safe for SSR
  ...
  const getSlideInfo = (s: Slide) => {
    if (s.type === "excursion" && s._excursion) {
      return { img: s._excursion.photos?.[0]||FALLBACK, title: s.custom_title||s._excursion.title, sub: s.custom_subtitle||s._excursion.city, color: s.custom_color||COLORS[0], tag: s._excursion.categories?.[0]||"Excursion" };
    }
    if (s.type === "video") {
  ...
  /* ── Preview panel (reusable for sidebar + mobile sheet) ── */
  const PreviewPanel = () => (
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',system-ui,sans-serif", background:"#F8FAFF", minHeight:"100vh" }}>
      ...
      {/* ── Layout ── */}
      <div className="main-area layout-grid" style={{ maxWidth:1100, margin:"0 auto", padding:"28px 32px 80px", display:"grid", gridTemplateColumns: showPreview ? "1fr 400px" : "1fr", gap:24 }}>

        {/* ══ LEFT — Slides list ══ */}
        <div>
          {/* Slides */}
          {slides.length === 0 ? (
            <div style={{ textAlign:"center", padding:"64px 20px", background:"white", borderRadius:20, border:"1.5px dashed #DCE5FF", marginBottom:20 }}>
                  ...
                  <div key={idx} className={`slide-card ${!slide.is_active?"inactive":""}`} style={{ animationDelay:`${idx*.04}s` }}>
                    ...
                    {isExpanded && (
                      <div style={{ padding:"14px 16px", borderTop:"1px solid #F3F4F6", background:"#FAFBFF", display:"flex", flexDirection:"column", gap:12 }}>
                        ...
                        {/* Move buttons (shown in expanded on mobile) */}
          ...
          {!addMode ? (
            <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
              <button className="type-tab" onClick={() => setAddMode("excursion")}><Star size={14}/> Ajouter une excursion</button>
              <button className="type-tab" onClick={() => setAddMode("video")}><Video size={14}/> Ajouter une vidéo</button>
              <button className="type-tab" onClick={() => setAddMode("custom")}><Image size={14}/> Image personnalisée</button>
            </div>
          ) : (
            <div style={{ background:"white", borderRadius:18, border:"1.5px solid #DCE5FF", padding:"18px" }}>
              {/* Form header */}
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                <div className="add-form-type-tabs" style={{ display:"flex", gap:8 }}>
                ...
                <button onClick={() => setAddMode(null)}
                  style={{ background:"#F3F4F6", border:"none", width:30, height:30, borderRadius:"50%", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"#9CA3AF", flexShrink:0 }}>
                  <X size={14}/>
                </button>
              </div>
      ...
      {/* ══ MOBILE preview sheet (shown via button, hides sidebar) ══ */}
      {showPreview && (
        <>
          {/* Only visible on ≤ 900px */}
          <style>{`
